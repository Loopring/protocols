...
contract("BatchTransfer", async accounts => {
  ...
  before(async () => {
    ...
    await lrcToken.transfer(batchTransfer.address, toContractAmount, {
      from: owner
    });
  });
  ...
  it("owner should be able to set lrc token address", async () => {
    ...
    assert.equal(
      zeroAddress,
      lrcAddressBefore,
      "lrcAddress initial value not zero address"
    );
  ...
  it("owner do batch transfer via LrcToken's batchTransfer function", async () => {
    ...
    for (let i = 0; i < 200; i++) {
      const accountNew = web3.eth.accounts.create().address;
    ...
    console.log(
      "\x1b[46m%s\x1b[0m",
      "gas used by token's batchTransfer: " + tx.receipt.gasUsed
    );
  ...
  it("owner should be able to do batch token transfer", async () => {
    ...
    const tx = await batchTransfer.doBatchTransfer(users, amounts, {
      from: owner
    });
  ...
  it("owner should be able to withdraw token in BatchTransfer contract", async () => {
    ...
    await batchTransfer.withdrawToken(lrcToken.address, amount, {
      from: owner
    });
    const balanceBN = await lrcToken.balanceOf(owner);
    assert(
      balanceBN.sub(balanceBefore).eq(amount),
      "lrc withdrawn amount not match"
    );
  });

  it("any other user should not be able to set lrc token address", async () => {
    try {
      await batchTransfer.setLrcAddress(lrcToken.address, {
        from: accounts[1]
      });
      assert(false);
    } catch (err) {
