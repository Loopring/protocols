...
export function toFloat(value: BN, encoding: FloatEncoding) {
  ...
  assert(value.lte(maxValue), "Value too large: " + value.toString(10) + ", max value: " + maxValue.toString(10));

  let exponent = 0;
  let r = value.div(new BN(maxMantissa));
  let d = new BN(1);
  while (r.gte(new BN(encoding.exponentBase)) || d.mul(new BN(maxMantissa)).lt(value)) {
