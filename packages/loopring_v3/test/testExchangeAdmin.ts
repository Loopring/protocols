...
contract("Exchange", (accounts: string[]) => {
  ...
  const setFeesChecked = async (accountCreationFeeETH: BN,
    ...
    assert(fees._accountCreationFeeETH.eq(accountCreationFeeETH), "accountCreationFeeETH does not match");
    assert(fees._accountUpdateFeeETH.eq(accountUpdateFeeETH), "accountUpdateFeeETH does not match");
    assert(fees._depositFeeETH.eq(depositFeeETH), "depositFeeETH does not match");
    assert(fees._withdrawalFeeETH.eq(withdrawalFeeETH), "withdrawalFeeETH does not match");
  };
  ...
  describe("Admin", function() {
    ...
    describe("Exchange owner", () => {
      ...
      it("should not be able to set the withdrawal fee higher than allowed", async () => {
        ...
        assert(!maxWithdrawalFee.isZero(), "max withdrawal fee cannot be 0 for test");
        await expectThrow(
          exchange.setFees(
            new BN(0),
            new BN(0),
