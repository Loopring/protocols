...
export class Simulator {
  ...
  public cancelOrder(
    ...
    // Update balance
    account.balances[orderTokenID].balance = account.balances[
      orderTokenID
    ].balance.sub(fee);
    account.nonce++;
  ...
  public settleRing(
    ...
    // - FeeA
    ringMatcher.balances[tokenB].balance = ringMatcher.balances[tokenB].balance
      .add(s.feeA)
      .sub(s.protocolFeeA)
      .sub(s.rebateA);
    // - FeeB
    ringMatcher.balances[tokenA].balance = ringMatcher.balances[tokenA].balance
      .add(s.feeB)
      .sub(s.protocolFeeB)
      .sub(s.rebateB);
    // - Operator fee
    ringMatcher.balances[feeToken].balance = ringMatcher.balances[
      feeToken
    ].balance.sub(ringFee);
    // Increase nonce
    ringMatcher.nonce++;
    ...
    operator.balances[feeToken].balance = operator.balances[
      feeToken
    ].balance.add(ringFee);
