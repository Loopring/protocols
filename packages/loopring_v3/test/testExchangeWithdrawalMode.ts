...
contract("Exchange", (accounts: string[]) => {
  ...
  describe("Withdrawal Mode", function() {
    ...
    it("ERC20: withdraw from merkle tree", async () => {
      ...
      const proof = await exchangeTestUtil.createMerkleTreeInclusionProof(owner, token);
    ...
    it("Should not be able to do any more block state changes", async () => {
      ...
      await expectThrow(
        exchange.commitBlock(0, 2, 0, web3.utils.hexToBytes("0x0"), web3.utils.hexToBytes("0x"),
