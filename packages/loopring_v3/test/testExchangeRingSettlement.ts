    ...
    it("Trimmed OrderID (order.orderID > tradeHistory.orderID)", async () => {
      ...
      const ringD: RingInfo = {
        ...
        orderB: {
          ...
          amountB: new BN(web3.utils.toWei("100", "ether")),
          owner: ownerB,
          orderID: orderIDB + 2 * 2 ** Constants.BINARY_TREE_DEPTH_TRADING_HISTORY
        },
        expected: {
          orderA: { filledFraction: 0.0, spread: new BN(0) },
          orderB: { filledFraction: 0.0 }
      ...
      const ringE: RingInfo = {
        orderA: {
          ...
          amountB: new BN(web3.utils.toWei("200", "ether")),
          owner: ownerA,
          orderID: orderIDA + 2 * 2 ** Constants.BINARY_TREE_DEPTH_TRADING_HISTORY
        },
        orderB: {
          tokenS: "GTO",
          tokenB: "WETH",
          ...
          amountB: new BN(web3.utils.toWei("100", "ether")),
          owner: ownerB,
          orderID: orderIDB + 3 * 2 ** Constants.BINARY_TREE_DEPTH_TRADING_HISTORY
        },
        expected: {
          orderA: { filledFraction: 1.0, spread: new BN(0) },
          orderB: { filledFraction: 1.0 }
    ...
    it("Invalid OrderID (order.orderID > tradeHistory.orderID)", async () => {
      ...
      const ringB: RingInfo = {
        orderA: {
          ...
          amountB: new BN(web3.utils.toWei("200", "ether")),
          owner: exchangeTestUtil.testContext.orderOwners[0],
          orderID: orderID + 2 * 2 ** Constants.BINARY_TREE_DEPTH_TRADING_HISTORY
        },
        orderB: {
          tokenS: "GTO",
          tokenB: "WETH",
