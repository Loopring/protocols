...
contract("LoopringV3Owner", (accounts: string[]) => {
  ...
  // USD values
  let dexStakingPer1000Blocks = new BN(web3.utils.toWei("10000", "ether"));
  ...
  const doLrcPriceChange = async (
    ...
    const startdexStakingPer1000BlocksLrc = await loopring.dexStakingPer1000Blocks();
    ...
    const expecteddexStakingPer1000BlocksLrc = transformPrice(
      startdexStakingPer1000BlocksLrc
    );
    ...
    await setConversion(
      ...
      expecteddexStakingPer1000BlocksLrc
    );
    ...
    assert(
      expecteddexStakingPer1000BlocksLrc.eq(
        await loopring.dexStakingPer1000Blocks()
      ),
      "unexpected currentdexStakingPer1000BlocksLrc"
    );
    ...
    assert(
      updateEvent.dexStakingPer1000BlocksLRC.eq(
        expecteddexStakingPer1000BlocksLrc
      ),
      "unexpected currentdexStakingPer1000BlocksLrc"
    );
