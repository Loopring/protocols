...
class State(object):
    ...
    def executeTransaction(self, context, txInput):
        ...
        # Tokens
        newState.BALANCE_A_S_ADDRESS = None
        newState.BALANCE_A_S_ADDRESS = None
        # A
        newState.ACCOUNT_A_ADDRESS = None
        newState.ACCOUNT_A_OWNER = None
        newState.ACCOUNT_A_PUBKEY_X = None
        newState.ACCOUNT_A_PUBKEY_Y = None
        newState.ACCOUNT_A_NONCE = None
        newState.BALANCE_A_S_ADDRESS = None
        newState.BALANCE_A_S_BALANCE = None
        newState.BALANCE_A_B_BALANCE = None
        newState.STORAGE_A_ADDRESS = None
        newState.STORAGE_A_DATA = None
        newState.STORAGE_A_STORAGEID = None
        # B
        newState.ACCOUNT_B_ADDRESS = None
        newState.ACCOUNT_B_OWNER = None
        newState.ACCOUNT_B_PUBKEY_X = None
        newState.ACCOUNT_B_PUBKEY_Y = None
        newState.ACCOUNT_B_NONCE = None
        newState.BALANCE_B_S_ADDRESS = None
        newState.BALANCE_B_S_BALANCE = None
        newState.BALANCE_B_B_BALANCE = None
        newState.STORAGE_B_ADDRESS = None
        newState.STORAGE_B_DATA = None
        newState.STORAGE_B_STORAGEID = None
        # Operator
        ...
        if txInput.txType == "Noop":
            ...
            newState.ACCOUNT_A_ADDRESS = ring.orderA.accountID
            accountA = self.getAccount(ring.orderA.accountID)

            newState.BALANCE_A_S_ADDRESS = ring.orderA.tokenS
            newState.BALANCE_A_S_BALANCE = -fillA.S

            newState.BALANCE_B_S_ADDRESS = ring.orderA.tokenB
            newState.BALANCE_A_B_BALANCE = fillA.B - fee_A

            newState.STORAGE_A_ADDRESS = ring.orderA.storageID
            newState.STORAGE_A_DATA = filled_A + (fillA.B if ring.orderA.fillAmountBorS else fillA.S)
            newState.STORAGE_A_STORAGEID = ring.orderA.storageID


            newState.ACCOUNT_B_ADDRESS = ring.orderB.accountID
            accountB = self.getAccount(ring.orderB.accountID)

            newState.BALANCE_B_S_ADDRESS = ring.orderB.tokenS
            newState.BALANCE_B_S_BALANCE = -fillB.S

            newState.BALANCE_A_S_ADDRESS = ring.orderB.tokenB
            newState.BALANCE_B_B_BALANCE = fillB.B - fee_B

            newState.STORAGE_B_ADDRESS = ring.orderB.storageID
            newState.STORAGE_B_DATA = filled_B + (fillB.B if ring.orderB.fillAmountBorS else fillB.S)
            newState.STORAGE_B_STORAGEID = ring.orderB.storageID
            ...
            newState.ACCOUNT_A_ADDRESS = txInput.fromAccountID
            accountA = self.getAccount(newState.ACCOUNT_A_ADDRESS)

            newState.BALANCE_A_S_ADDRESS = txInput.tokenID
            newState.BALANCE_A_S_BALANCE = -transferAmount

            newState.BALANCE_B_S_ADDRESS = txInput.feeTokenID
            newState.BALANCE_A_B_BALANCE = -feeValue

            newState.ACCOUNT_B_ADDRESS = txInput.toAccountID
            accountB = self.getAccount(newState.ACCOUNT_B_ADDRESS)
            newState.ACCOUNT_B_OWNER = txInput.to

            newState.BALANCE_A_S_ADDRESS = txInput.tokenID
            newState.BALANCE_B_B_BALANCE = transferAmount

            newState.STORAGE_A_ADDRESS = txInput.storageID
            newState.STORAGE_A_DATA = 1
            newState.STORAGE_A_STORAGEID = txInput.storageID
            ...
            newState.ACCOUNT_A_ADDRESS = 1 if isProtocolfeeWithdrawal else txInput.accountID
            accountA = self.getAccount(newState.ACCOUNT_A_ADDRESS)

            newState.BALANCE_A_S_ADDRESS = txInput.tokenID
            newState.BALANCE_A_S_BALANCE = 0 if isProtocolfeeWithdrawal else -int(txInput.amount)

            newState.BALANCE_B_S_ADDRESS = txInput.feeTokenID
            newState.BALANCE_A_B_BALANCE = -feeValue

            if int(txInput.type) == 0 or int(txInput.type) == 1:
                newState.ACCOUNT_A_NONCE = 1
            ...
            newState.ACCOUNT_A_ADDRESS = txInput.accountID
            newState.ACCOUNT_A_OWNER = txInput.owner

            newState.BALANCE_A_S_ADDRESS = txInput.tokenID
            newState.BALANCE_A_S_BALANCE = txInput.amount
            ...
            newState.ACCOUNT_A_PUBKEY_X = txInput.publicKeyX
            newState.ACCOUNT_A_PUBKEY_Y = txInput.publicKeyY
            newState.ACCOUNT_A_NONCE = 1

            newState.BALANCE_A_S_ADDRESS = txInput.feeTokenID
            newState.BALANCE_A_S_BALANCE = -feeValue
        ...
        newState.BALANCE_A_S_ADDRESS = setValue(newState.BALANCE_A_S_ADDRESS, 0)
        newState.BALANCE_B_S_ADDRESS = setValue(newState.BALANCE_B_S_ADDRESS, 0)

        # A default values
        newState.ACCOUNT_A_ADDRESS = setValue(newState.ACCOUNT_A_ADDRESS, 1)
        accountA = self.getAccount(newState.ACCOUNT_A_ADDRESS)
        newState.ACCOUNT_A_OWNER = setValue(newState.ACCOUNT_A_OWNER, accountA.owner)
        newState.ACCOUNT_A_PUBKEY_X = setValue(newState.ACCOUNT_A_PUBKEY_X, accountA.publicKeyX)
        newState.ACCOUNT_A_PUBKEY_Y = setValue(newState.ACCOUNT_A_PUBKEY_Y, accountA.publicKeyY)
        newState.ACCOUNT_A_NONCE = setValue(newState.ACCOUNT_A_NONCE, 0)

        balanceLeafA_S = accountA.getBalanceLeaf(newState.BALANCE_A_S_ADDRESS)
        newState.BALANCE_A_S_BALANCE = setValue(newState.BALANCE_A_S_BALANCE, 0)

        newState.BALANCE_A_B_BALANCE = setValue(newState.BALANCE_A_B_BALANCE, 0)

        newState.STORAGE_A_ADDRESS = setValue(newState.STORAGE_A_ADDRESS, 0)
        storageA = balanceLeafA_S.getStorage(newState.STORAGE_A_ADDRESS)
        newState.STORAGE_A_DATA = setValue(newState.STORAGE_A_DATA, storageA.data)
        newState.STORAGE_A_STORAGEID = setValue(newState.STORAGE_A_STORAGEID, storageA.storageID)
        ...
        accountBefore = copyAccountInfo(self.getAccount(newState.ACCOUNT_A_ADDRESS))
        proof = self._accountsTree.createProof(newState.ACCOUNT_A_ADDRESS)

        (balanceUpdateS_A, storageUpdate_A) = accountA.updateBalanceAndStorage(
            newState.BALANCE_A_S_ADDRESS,
            newState.STORAGE_A_STORAGEID,
            newState.STORAGE_A_DATA,
            newState.BALANCE_A_S_BALANCE
        )
        balanceUpdateB_A = accountA.updateBalance(
            newState.BALANCE_B_S_ADDRESS,
            newState.BALANCE_A_B_BALANCE
        )

        accountA.owner = newState.ACCOUNT_A_OWNER
        accountA.publicKeyX = newState.ACCOUNT_A_PUBKEY_X
        accountA.publicKeyY = newState.ACCOUNT_A_PUBKEY_Y
        accountA.nonce = accountA.nonce + newState.ACCOUNT_A_NONCE

        self.updateAccountTree(newState.ACCOUNT_A_ADDRESS)
        accountAfter = copyAccountInfo(self.getAccount(newState.ACCOUNT_A_ADDRESS))
        rootAfter = self._accountsTree._root
        accountUpdate_A = AccountUpdateData(newState.ACCOUNT_A_ADDRESS, proof, rootBefore, rootAfter, accountBefore, accountAfter)
        ###

        # B default values
        newState.ACCOUNT_B_ADDRESS = setValue(newState.ACCOUNT_B_ADDRESS, 1)
        accountB = self.getAccount(newState.ACCOUNT_B_ADDRESS)
        newState.ACCOUNT_B_OWNER = setValue(newState.ACCOUNT_B_OWNER, accountB.owner)
        newState.ACCOUNT_B_PUBKEY_X = setValue(newState.ACCOUNT_B_PUBKEY_X, accountB.publicKeyX)
        newState.ACCOUNT_B_PUBKEY_Y = setValue(newState.ACCOUNT_B_PUBKEY_Y, accountB.publicKeyY)
        newState.ACCOUNT_B_NONCE = setValue(newState.ACCOUNT_B_NONCE, 0)

        balanceLeafB_S = accountB.getBalanceLeaf(newState.BALANCE_B_S_ADDRESS)
        newState.BALANCE_B_S_BALANCE = setValue(newState.BALANCE_B_S_BALANCE, 0)

        newState.BALANCE_B_B_BALANCE = setValue(newState.BALANCE_B_B_BALANCE, 0)

        newState.STORAGE_B_ADDRESS = setValue(newState.STORAGE_B_ADDRESS, 0)
        storageB = balanceLeafB_S.getStorage(newState.STORAGE_B_ADDRESS)
        newState.STORAGE_B_DATA = setValue(newState.STORAGE_B_DATA, storageB.data)
        newState.STORAGE_B_STORAGEID = setValue(newState.STORAGE_B_STORAGEID, storageB.storageID)

        # Update B
        accountB = self.getAccount(newState.ACCOUNT_B_ADDRESS)

        rootBefore = self._accountsTree._root
        accountBefore = copyAccountInfo(self.getAccount(newState.ACCOUNT_B_ADDRESS))
        proof = self._accountsTree.createProof(newState.ACCOUNT_B_ADDRESS)

        (balanceUpdateS_B, storageUpdate_B) = accountB.updateBalanceAndStorage(
            newState.BALANCE_B_S_ADDRESS,
            newState.STORAGE_B_STORAGEID,
            newState.STORAGE_B_DATA,
            newState.BALANCE_B_S_BALANCE
        )
        balanceUpdateB_B = accountB.updateBalance(
            newState.BALANCE_A_S_ADDRESS,
            newState.BALANCE_B_B_BALANCE
        )

        accountB.owner = newState.ACCOUNT_B_OWNER
        accountB.publicKeyX = newState.ACCOUNT_B_PUBKEY_X
        accountB.publicKeyY = newState.ACCOUNT_B_PUBKEY_Y
        accountB.nonce = accountB.nonce + newState.ACCOUNT_B_NONCE

        self.updateAccountTree(newState.ACCOUNT_B_ADDRESS)
        accountAfter = copyAccountInfo(self.getAccount(newState.ACCOUNT_B_ADDRESS))
        rootAfter = self._accountsTree._root
        accountUpdate_B = AccountUpdateData(newState.ACCOUNT_B_ADDRESS, proof, rootBefore, rootAfter, accountBefore, accountAfter)
        ###
        ...
        balanceUpdateB_P = self.getAccount(0).updateBalance(newState.BALANCE_A_S_ADDRESS, newState.balanceDeltaB_P)
        balanceUpdateA_P = self.getAccount(0).updateBalance(newState.BALANCE_B_S_ADDRESS, newState.balanceDeltaA_P)
        ###
