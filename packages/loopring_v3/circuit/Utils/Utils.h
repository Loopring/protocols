...
static void print(const ProtoboardT& pb, const char* description,
                  const ethsnarks::VariableT& variable) {
  print(description, pb.val(variable));
}

static void printBits(const char* name, const libff::bit_vector& _bits,
                      bool reverse = false) {
 ...
 */
static const VariableArrayT flattenReverse(
    const std::vector<VariableArrayT>& in_scalars) {
  size_t total_sz = 0;
  for (const auto& scalar : in_scalars) total_sz += scalar.size();
...
static const VariableArrayT subArray(const VariableArrayT& bits,
...
static unsigned int toFloat(BigInt value, const FloatEncoding& encoding) {
  ...
  const unsigned int f =
      (exponent << encoding.numBitsMantissa) + mantissa.to_long();
  return f;
}

static unsigned int toFloat(ethsnarks::FieldT value,
                            const FloatEncoding& encoding) {
...
static FieldT roundToFloatValue(const FieldT& value,
                                const FloatEncoding& encoding) {
