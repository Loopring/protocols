...
// Checks that the new owner equals the current onwer or the current owner is 0.
...
// Checks if a tokenID is used for an NFT or not
...
// Requires that the tokenID is not used for an NFT
...
// Checks ERC20/NFT token transfer requirements.
    ...
    TokenTransferDataGadget(
      ...
      const std::string &prefix)
        : GadgetT(pb, prefix),
          ...
          tokensEqual(pb, fromTokenID, toTokenID, FMT(prefix, ".tokensEqual")),

          // NFT data values in the NFT from/to token slots need to match for an NFT transfer
          // (unless the to token slot is currently empty)
          nftDataValid(pb, constants, toNftData, fromNftData, isNftFromTokenID.isNFT(), FMT(prefix, ".nftDataValid")),

          // From/To token slots need to be equal for an ERC20 transfer
          nonNftRequirement(pb, {isNftFromTokenID.isNotNFT(), tokensEqual.result()}, FMT(prefix, ".nonNftRequirement")),
          // From/To token slots both need to be NFT token slots for an NFT transfer
          nftRequirement(pb, {isNftFromTokenID.isNFT(), isNftToTokenID.isNFT()}, FMT(prefix, ".nftRequirement")),

          // Needs to be valid ERC20 transfer or a valid NFT transfer
          ...
          // From NftData field will be reset to 0 if all NFTs have been transferred out,
          // otherwise the value should remain the same.
          ...
          // To NftData field will be copied from the from NftData field for NFT transfers
          toNewNftData(pb, isNftToTokenID.isNFT(), fromNftData, toNftData, FMT(prefix, ".toNewNftData")),

          // Put the to token slot in the DA if it's different from the from token slot
...
// Hashes all the NFT data to the NftData value
