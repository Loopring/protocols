...
class OrGadget : public GadgetT {
 public:
  ...
  void generate_r1cs_constraints() {
    pb.add_r1cs_constraint(
        ConstraintT(FieldT::one() - inputs[0], FieldT::one() - inputs[1],
                    FieldT::one() - results[0]),
        FMT(annotation_prefix, ".A || B == _or"));
    for (unsigned int i = 2; i < inputs.size(); i++) {
      pb.add_r1cs_constraint(
          ConstraintT(FieldT::one() - inputs[i], FieldT::one() - results[i - 2],
  ...
  void generate_r1cs_constraints() {
    ...
    for (unsigned int i = floatEncoding.numBitsMantissa; i < f.size(); i++) {
      ...
      if (j == 0) {
        ...
        pb.add_r1cs_constraint(
            ConstraintT(baseMultipliers[j - 1], baseMultipliers[j - 1],
                        baseMultipliers[j]),
            ".baseMultipliers");
      }
