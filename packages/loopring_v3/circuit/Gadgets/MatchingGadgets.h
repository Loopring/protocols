  ...
  CheckValidGadget(
    ...
    const std::string &prefix)
      : GadgetT(pb, prefix),
        ...
        notValidAllOrNoneSell(
          pb,
          {order.allOrNone.packed, order_sell.result(), fillAmountS_lt_amountS.lt()},
          FMT(prefix, ".notValidAllOrNoneSell")),
        notValidAllOrNoneBuy(
          pb,
          {order.allOrNone.packed, order.buy.packed, fillAmountB_lt_amountB.lt()},
          FMT(prefix, ".notValidAllOrNoneBuy")),

        validSince_leq_timestamp(
          pb,
          order.validSince.packed,
          timestamp,
          NUM_BITS_TIMESTAMP,
          FMT(prefix, "validSince <= timestamp")),
        timestamp_leq_validUntil(
          pb,
          timestamp,
...
public:
  ...
  const VariableT &getFilledAfter() const { return requireFillLimit.getFilledAfter(); }
};
...
public:
  ...
  const VariableT &getFilledAfter_A() const { return requireOrderFillsA.getFilledAfter(); }

  const VariableT &getFilledAfter_B() const { return requireOrderFillsB.getFilledAfter(); }
};
