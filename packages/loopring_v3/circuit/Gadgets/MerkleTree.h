    ...
    for (size_t i = 0; i < in_tree_depth; i++)
      {
        m_selectors.push_back(merkle_path_selector(
          in_pb,
          (i == 0) ? in_leaf_hash : m_hashers[i - 1].result(),
 ...
 */
template <typename HashT> class merkle_root_verifier : public merkle_root_updater<HashT>
{
public:
  const VariableT m_expected_root;

  merkle_root_verifier(
    ProtoboardT &in_pb,
    const size_t in_tree_depth,
    const VariableArrayT in_address_bits,
