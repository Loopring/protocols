    ...
    NftMintCircuit( //
      ...
      const std::string &prefix)
        : BaseTransactionCircuit(pb, state, prefix),
          ...
          // To address needs to match account B's owner for conditional mints
          toOwnerValid(
            pb,
            state.constants,
          ...
          // Non-conditional mints can only be down to the minter's own account
          requireToSelf(
            pb,
          ...
          // We do not allow L2 mints to have tokenAddress == minter (which is reserved for deposits)
          ...
          // Require tokenAddress == accountB.owner for non-conditional mints
          ...
          // Type differences
          // For conditional mints the NFT is minted to accountB in token slot S,
          // for non-conditional mints the NFT is minted to accountA in token slot B
          accountB_address(
            pb,
            isConditional.result(),
            toAccountID.bits,
          ...
          // Nonce (not increased for deposits)
          nonce(
            pb,
            state.constants,
