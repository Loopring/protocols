...
public:
  ...
  SelectTransactionGadget(
      ProtoboardT &pb, const TransactionState &state,
    ...
    for (const auto &uPair : uOutputs) {
      ...
      uSelects.emplace_back(pb, selector, variables,
    ...
    for (const auto &aPair : aOutputs) {
      ...
      aSelects.emplace_back(pb, selector, variables,
    ...
    // Public data
    {
      ...
      for (unsigned int i = 0; i < transactions.size(); i++) {
        ...
        for (unsigned int j = da.size();
             j < (TX_DATA_AVAILABILITY_SIZE - 1) * 8; j++) {
      ...
      publicDataSelects.emplace_back(
          pb, selector, variables,
          FMT(annotation_prefix, ".publicDataSelects"));
    }
  }
...
public:
  ...
  TransactionGadget(ProtoboardT &pb, const jubjub::Params &params,
                    const Constants &_constants, const VariableT &exchange,
                    const VariableT &accountsRoot, const VariableT &timestamp,
      ...
      : GadgetT(pb, prefix),
        ...
        selector(pb, constants, type.packed,
                 (unsigned int)TransactionType::COUNT,
                 FMT(prefix, ".selector")),

        state(pb, params, constants, exchange, timestamp, protocolTakerFeeBips,
              protocolMakerFeeBips, numConditionalTransactionsBefore,
              type.packed, FMT(prefix, ".transactionState")),
        ...
        tx(pb, state, selector.result(),
           {&noop, &deposit, &withdraw, &transfer, &spotTrade, &accountUpdate},
           FMT(prefix, ".tx")),

        // General validation
        accountA(pb, tx.getArrayOutput(accountA_Address),
                 FMT(prefix, ".packAccountA")),
        accountB(pb, tx.getArrayOutput(accountB_Address),
                 FMT(prefix, ".packAccountA")),
        ...
        // Check signatures
        signatureVerifierA(pb, params, state.constants,
                           jubjub::VariablePointT(tx.getOutput(publicKeyX_A),
                                                  tx.getOutput(publicKeyY_A)),
                           tx.getOutput(hash_A),
                           tx.getOutput(signatureRequired_A),
                           FMT(prefix, ".signatureVerifierA")),
        signatureVerifierB(pb, params, state.constants,
                           jubjub::VariablePointT(tx.getOutput(publicKeyX_B),
                                                  tx.getOutput(publicKeyY_B)),
                           tx.getOutput(hash_B),
        ...
        updateBalanceB_A(pb, updateBalanceS_A.result(),
                         tx.getArrayOutput(balanceB_S_Address),
                         {state.accountA.balanceB.balance,
                          state.accountA.balanceB.storageRoot},
                         {tx.getOutput(balanceA_B_Balance),
                          state.accountA.balanceB.storageRoot},
                         FMT(prefix, ".updateBalanceB_A")),
        updateAccount_A(
            pb, accountsRoot, tx.getArrayOutput(accountA_Address),
            {state.accountA.account.owner, state.accountA.account.publicKey.x,
             state.accountA.account.publicKey.y, state.accountA.account.nonce,
             state.accountA.account.balancesRoot},
            {tx.getOutput(accountA_Owner), tx.getOutput(accountA_PublicKeyX),
             tx.getOutput(accountA_PublicKeyY), tx.getOutput(accountA_Nonce),
        ...
        updateBalanceB_B(pb, updateBalanceS_B.result(),
                         tx.getArrayOutput(balanceA_S_Address),
                         {state.accountB.balanceB.balance,
                          state.accountB.balanceB.storageRoot},
                         {tx.getOutput(balanceB_B_Balance),
                          state.accountB.balanceB.storageRoot},
                         FMT(prefix, ".updateBalanceB_B")),
        updateAccount_B(
            pb, updateAccount_A.result(), tx.getArrayOutput(accountB_Address),
            {state.accountB.account.owner, state.accountB.account.publicKey.x,
             state.accountB.account.publicKey.y, state.accountB.account.nonce,
             state.accountB.account.balancesRoot},
            {tx.getOutput(accountB_Owner), tx.getOutput(accountB_PublicKeyX),
             tx.getOutput(accountB_PublicKeyY), tx.getOutput(accountB_Nonce),
        ...
        updateAccount_O(
            ...
            {state.oper.account.owner, state.oper.account.publicKey.x,
             state.oper.account.publicKey.y, state.oper.account.nonce,
        ...
        // Update Protocol pool
        updateBalanceB_P(
            pb, protocolBalancesRoot, tx.getArrayOutput(balanceA_S_Address),
  ...
  void generate_r1cs_witness(const UniversalTransaction &uTx) {
    ...
    state.generate_r1cs_witness(
        uTx.witness.accountUpdate_A.before, uTx.witness.balanceUpdateS_A.before,
        uTx.witness.balanceUpdateB_A.before, uTx.witness.storageUpdate_A.before,
        uTx.witness.accountUpdate_B.before, uTx.witness.balanceUpdateS_B.before,
        uTx.witness.balanceUpdateB_B.before, uTx.witness.storageUpdate_B.before,
        uTx.witness.accountUpdate_O.before, uTx.witness.balanceUpdateA_O.before,
...
public:
  ...
  std::unique_ptr<libsnark::dual_variable_gadget<FieldT>>
  ...
  UniversalCircuit(ProtoboardT &pb, const std::string &prefix)
      : Circuit(pb, prefix),
        ...
        protocolTakerFeeBips(pb, NUM_BITS_PROTOCOL_FEE_BIPS,
                             FMT(prefix, ".protocolTakerFeeBips")),
        protocolMakerFeeBips(pb, NUM_BITS_PROTOCOL_FEE_BIPS,
                             FMT(prefix, ".protocolMakerFeeBips")),
        operatorAccountID(pb, NUM_BITS_ACCOUNT,
                          FMT(prefix, ".operatorAccountID")),

        // Increment the nonce of the Operator
        nonce_after(pb, accountBefore_O.nonce, constants._1, NUM_BITS_NONCE,
                    FMT(prefix, ".nonce_after")),

        // Signature
        hash(pb, var_array({publicData.publicInput, accountBefore_O.nonce}),
             FMT(this->annotation_prefix, ".hash")),
        signatureVerifier(pb, params, constants, accountBefore_O.publicKey,
                          hash.result(), constants._1,
  ...
  void generateConstraints(unsigned int blockSize) override {
    ...
    for (size_t j = 0; j < numTransactions; j++) {
      ...
      const VariableT &txProtocolBalancesRoot =
          (j == 0) ? accountBefore_P.balancesRoot
                   : transactions.back().getNewProtocolBalancesRoot();
      transactions.emplace_back(pb, params, constants, exchange.packed,
                                txAccountsRoot, timestamp.packed,
                                protocolTakerFeeBips.packed,
                                protocolMakerFeeBips.packed,
                                operatorAccountID.bits, txProtocolBalancesRoot,
                                (j == 0) ? constants._0
                                         : transactions.back().tx.getOutput(
    ...
    updateAccount_P.reset(new UpdateAccountGadget(
        pb, transactions.back().getNewAccountsRoot(), constants.zeroAccount,
        {accountBefore_P.owner, accountBefore_P.publicKey.x,
         accountBefore_P.publicKey.y, accountBefore_P.nonce,
    ...
    updateAccount_O.reset(new UpdateAccountGadget(
        pb, updateAccount_P->result(), operatorAccountID.bits,
        {accountBefore_O.owner, accountBefore_O.publicKey.x,
         accountBefore_O.publicKey.y, accountBefore_O.nonce,
         accountBefore_O.balancesRoot},
        {accountBefore_O.owner, accountBefore_O.publicKey.x,
         accountBefore_O.publicKey.y, nonce_after.result(),
         accountBefore_O.balancesRoot},
    ...
    numConditionalTransactions.reset(new libsnark::dual_variable_gadget<FieldT>(
        pb, transactions.back().tx.getOutput(misc_NumConditionalTransactions),
        32, ".numConditionalTransactions"));
    ...
    requireEqual(pb, updateAccount_O->result(), merkleRootAfter.packed,
                 "newMerkleRoot");
  }
...
#pragma omp parallel for
  ...
  unsigned int getBlockType() override { return 0; }

  unsigned int getBlockSize() override { return numTransactions; }

  void printInfo() override {
    std::cout << pb.num_constraints() << " constraints ("
              << (pb.num_constraints() / numTransactions) << "/tx)"
              << std::endl;
  }
};
