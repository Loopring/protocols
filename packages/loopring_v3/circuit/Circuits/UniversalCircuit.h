...
public:
      ...
      : BaseTransactionCircuit(pb, state, prefix)
  {
    ...
    // Public data
    {
      ...
      publicDataSelects.emplace_back(
        pb, selector, variables, FMT(annotation_prefix, ".publicDataSelects"));
    }
  }
  ...
  const VariableArrayT getPublicData() const { return publicDataSelects.back().result(); }
};
...
public:
    ...
    const std::string &prefix)
      : GadgetT(pb, prefix),
        ...
        updateBalanceB_O(
          ...
          {tx.getOutput(BALANCE_O_B_BALANCE), state.oper.balanceB.storageRoot},
        ...
        updateBalanceA_O(
          ...
          {tx.getOutput(BALANCE_O_A_BALANCE), state.oper.balanceA.storageRoot},
  ...
  const VariableT &getNewAccountsRoot() const { return updateAccount_O.result(); }

  const VariableT &getNewProtocolBalancesRoot() const { return updateBalanceA_P.result(); }
};
...
public:
  ...
  UniversalCircuit(ProtoboardT &pb, const std::string &prefix)
      : Circuit(pb, prefix),
        ...
        hash(
          pb,
          var_array({publicData.publicInput, accountBefore_O.nonce}),
  ...
  void generateConstraints(unsigned int blockSize) override
  {
    ...
    numConditionalTransactions.reset(new libsnark::dual_variable_gadget<FieldT>(
      pb,
      transactions.back().tx.getOutput(NUM_CONDITIONAL_TXS),
      32,
...
#pragma omp parallel for
  ...
  bool generateWitness(const json &input) override { return generateWitness(input.get<Block>()); }
  ...
  void printInfo() override
  {
    std::cout << pb.num_constraints() << " constraints ("
              << (pb.num_constraints() / numTransactions) << "/tx)" << std::endl;
  }
};
