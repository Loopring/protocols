...
Loopring::Circuit* createCircuit(unsigned int blockType, unsigned int blockSize, bool rollupEnabled, ethsnarks::ProtoboardT& outPb)
{
    std::cout << "Creating circuit... " << std::endl;
    auto begin = now();
    Loopring::Circuit* circuit = newCircuit(blockType, outPb);
    circuit->generateConstraints(rollupEnabled, blockSize);
    circuit->printInfo();
...
int main (int argc, char **argv)
{
    ...
    int iBlockType = input["blockType"].get<int>();
    unsigned int blockSize = input["blockSize"].get<int>();
    bool rollupEnabled = input["rollupEnabled"].get<bool>();
    std::string strrollupEnabled = rollupEnabled ? "_DA_" : "_";
    std::string postFix = strrollupEnabled + std::to_string(blockSize);
    ...
    Loopring::Circuit* circuit = createCircuit(blockType, blockSize, rollupEnabled, pb);
    if (config.swapAB)
    {
