...
void printMemoryUsage()
{
  ...
  std::cout << "VM: " << unsigned(vm * 0.001) << "MB; RSS: " << unsigned(rss * 0.001) << "MB"
            << std::endl;
}
#else
void printMemoryUsage() {}
...
{
  context.scratch_exponents.resize(
    std::max(context.constraint_system->num_variables() + 1, context.domain->m - 1));
...
bool generateKeyPair(ethsnarks::ProtoboardT &pb, std::string &baseFilename)
{
  ...
  int result =
    stub_genkeys_from_pb(pb, provingKeyFilename.c_str(), verificationKeyFilename.c_str());
#endif
  return (result == 0);
}
...
Loopring::Circuit *
createCircuit(unsigned int blockType, unsigned int blockSize, ethsnarks::ProtoboardT &outPb)
{
  ...
  unsigned int port)
{
  ...
  svr.Get("/prove", [&](const Request &req, Response &res) {
    ...
    if (
      /*iBlockType & circuit->getBlockType() != 1 || */ blockSize != circuit->getBlockSize())
      {
        res.set_content(
...
bool runBenchmark(Loopring::Circuit *circuit, const std::string &provingKeyFilename)
{
  ...
  struct Result
  {
    ...
    static bool compareResult(Result a, Result b) { return (a.duration_ms < b.duration_ms); }
  };
...
int main(int argc, char **argv)
{
  ...
  if (strcmp(argv[1], "-validate") == 0)
  ...
  else if (strcmp(argv[1], "-server") == 0)
    {
      ...
      mode = Mode::Server;
      std::cout << "Starting proving server for " << argv[2] << " on port " << argv[3] << "..."
                << std::endl;
    }
