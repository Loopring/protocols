...
void printMemoryUsage() {
  ...
  std::cout << "VM: " << unsigned(vm * 0.001)
            << "MB; RSS: " << unsigned(rss * 0.001) << "MB" << std::endl;
}
#else
void printMemoryUsage() {}
#endif
...
static void from_json(const nlohmann::json& j, libsnark::Config& config) {
  ...
  if (j.contains("multi_exp_prefetch_locality")) {
    config.multi_exp_prefetch_locality =
        j.at("multi_exp_prefetch_locality").get<unsigned int>();
  }
  ...
  if (j.contains("multi_exp_look_ahead")) {
    config.multi_exp_look_ahead =
        j.at("multi_exp_look_ahead").get<unsigned int>();
  }
}
}  // namespace libsnark
...
struct BenchmarkConfig {
  ...
  std::vector<unsigned int>
      multi_exp_prefetch_locality;  // 4 == no prefetching, [0, 3] prefetch
                                    // locality
...
static void from_json(const nlohmann::json& j, BenchmarkConfig& config) {
  ...
  config.radixes =
      j.at("radixes").get<std::vector<std::vector<unsigned int>>>();
  config.swapAB = j.at("swapAB").get<std::vector<bool>>();
  config.multi_exp_c = j.at("multi_exp_c").get<std::vector<unsigned int>>();
  config.multi_exp_prefetch_locality =
      j.at("multi_exp_prefetch_locality").get<std::vector<unsigned int>>();
  config.prefetch_stride =
      j.at("prefetch_stride").get<std::vector<unsigned int>>();
  config.multi_exp_look_ahead =
      j.at("multi_exp_look_ahead").get<std::vector<unsigned int>>();
}
...
void initProverContextBuffers(ProverContextT& context) {
  context.scratch_exponents.resize(std::max(
      context.constraint_system->num_variables() + 1, context.domain->m - 1));
...
bool generateKeyPair(ethsnarks::ProtoboardT& pb, std::string& baseFilename) {
...
#endif
  if (fileExists(provingKeyFilename.c_str()) &&
      fileExists(verificationKeyFilename.c_str())
#ifdef GPU_PROVE
  ...
  int result = stub_genkeys_params_from_pb(pb, provingKeyFilename.c_str(),
                                           verificationKeyFilename.c_str(),
                                           paramsFilename.c_str());
#else
  std::cout << "Generating keys..." << std::endl;
  int result = stub_genkeys_from_pb(pb, provingKeyFilename.c_str(),
                                    verificationKeyFilename.c_str());
#endif
  return (result == 0);
}
...
void loadProvingKey(const std::string& pk_file,
...
std::string proveCircuit(ProverContextT& context, Loopring::Circuit* circuit) {
  ...
  std::cout << "Proof generated in " << float(elapsed_ms) / 1000.0f
            << " seconds ("
            << (circuit->getPb().num_constraints() * 10) / (elapsed_ms / 100)
            << " constraints/second)" << std::endl;
  return jProof;
}
...
Loopring::Circuit* newCircuit(unsigned int blockType,
                              ethsnarks::ProtoboardT& outPb) {
  return new Loopring::UniversalCircuit(outPb, "circuit");
}

Loopring::Circuit* createCircuit(unsigned int blockType, unsigned int blockSize,
                                 ethsnarks::ProtoboardT& outPb) {
...
void runServer(Loopring::Circuit* circuit,
               const std::string& provingKeyFilename,
               const libsnark::Config& config, unsigned int port) {
  ...
  struct ProverStatusRAII {
    ...
    ProverStatusRAII(ProverStatus& _proverStatus,
    ...
    ~ProverStatusRAII() { proverStatus.proving = false; }
  };
  ...
  svr.Get("/prove", [&](const Request& req, Response& res) {
    ...
    if (/*iBlockType & circuit->getBlockType() != 1 || */ blockSize !=
      ...
      res.set_content(
          "Error: Incompatible block requested! Use /info to check which "
          "blocks can be proven.\n",
          "text/plain");
      return;
    }
  ...
  svr.Get("/info", [&](const Request& req, Response& res) {
    std::string info = std::string("BlockType: ") +
                       std::to_string(int(circuit->getBlockType())) +
  ...
  svr.Get("/", [&](const Request& req, Response& res) {
    ...
    content +=
        "- Status of the server: /status (busy proving a block or not)\n";
    content += "- Info of the server: /info (which blocks can be proven)\n";
    content +=
        "- Shut down the server: /stop (will first finish generating the proof "
        "if busy)\n";
...
bool runBenchmark(Loopring::Circuit* circuit,
                  const std::string& provingKeyFilename) {
  ...
  VerificationKeyT vk = loadVerificationKey(
      provingKeyFilename.substr(0, provingKeyFilename.length() - 6) +
      "vk.json");
  ...
  BenchmarkConfig benchmarkConfig =
      loadJSON("benchmark.json").get<BenchmarkConfig>();
          ...
          for (auto fft : benchmarkConfig.fft) {
            for (auto radixes : benchmarkConfig.radixes) {
              for (auto multi_exp_c : benchmarkConfig.multi_exp_c) {
                for (auto multi_exp_prefetch_locality :
                     benchmarkConfig.multi_exp_prefetch_locality) {
                  for (auto multi_exp_look_ahead :
                       benchmarkConfig.multi_exp_look_ahead) {
  ...
  for (auto config : configs) {
    ...
    for (unsigned int l = 0; l < num_iterations; l++) {
      ...
      if (!libsnark::r1cs_gg_ppzksnark_zok_verifier_strong_IC<ppT>(
  ...
  for (unsigned int i = 0; i < results.size(); i++) {
    ...
    std::cout << i << ". " << config << " (" << results[i].duration_ms << "ms)"
              << std::endl;
  }
...
int main(int argc, char** argv) {
  ...
  if (argc < 3) {
    ...
    std::cerr << "-prove <block.json> <out_proof.json>: Proves a block"
              << std::endl;
    std::cerr << "-createkeys <protoBlock.json>: Creates prover/verifier keys"
              << std::endl;
  ...
  if (strcmp(argv[1], "-validate") == 0) {
    ...
    std::cout << "Converting pk from " << argv[2] << " to " << argv[3] << " ..."
              << std::endl;
    ...
    mode = Mode::Server;
    std::cout << "Starting proving server for " << argv[2] << " on port "
              << argv[3] << "..." << std::endl;
  ...
  if (mode == Mode::Prove) {
    ...
    auto begin = now();
    stub_write_input_from_pb(pb, provingKeyFilename.c_str(),
