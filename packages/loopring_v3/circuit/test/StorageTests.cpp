...
TEST_CASE("StorageReader", "[StorageReaderGadget]") {
  auto storageReaderChecked =
      [](const StorageLeaf& storageLeaf, const FieldT& _storageID, bool _verify,
        ...
        VariableT verify = make_variable(
            pb, _verify ? FieldT::one() : FieldT::zero(), ".verify");
        ...
        if (expectedSatisfied) {
          ...
          REQUIRE((pb.val(storageReaderGadget.getOverwrite()) ==
                   expectedOverwrite));
        }
      };
  ...
  SECTION("storageID > leafStorageID (trimmed)") {
    ...
    SECTION("Previous order filled") {
      storageReaderChecked({filled, storageID}, delta + storageID, true, true,
                           0, 1);
    }
...
TEST_CASE("Nonce", "[NonceGadget]") {
  auto nonceChecked = [](const StorageLeaf& storageLeaf,
    ...
    VariableT verify =
        make_variable(pb, _verify ? FieldT::one() : FieldT::zero(), ".verify");
  ...
  SECTION("storageID > leafStorageID (trimmed)") {
    SECTION("First overwrite") { nonceChecked({0, 0}, delta, true, true, 1); }
  ...
  SECTION("storageID < leafStorageID (cancelled)") {
    SECTION("First rejection") { nonceChecked({0, delta * 2}, 0, true, false); }
