...
OrderState setOrderState(const OrderState &orderState, const FieldT &storageID,
                         const FieldT &amountS, const FieldT &amountB, bool fillAmountBorS,
...
bool lt(const FieldT &A, const FieldT &B) { return toBigInt(A) < toBigInt(B); }
...
TEST_CASE("RequireFillRate", "[RequireFillRateGadget]") {
  ...
  for (unsigned int n = 1; n <= maxLength; n++) {
    DYNAMIC_SECTION("Bit-length: " << n) {
      enum class Expected { Valid, Invalid, Automatic };
      auto checkFillRateChecked = [n](const BigInt &_amountS,
        ...
        VariableT amountS =
            make_variable(pb, toFieldElement(_amountS), "amountS");
        VariableT amountB =
            make_variable(pb, toFieldElement(_amountB), "amountB");
        VariableT fillAmountS =
            make_variable(pb, toFieldElement(_fillAmountS), "fillAmountS");
        ...
        bool expectedAutomaticValid = (_fillAmountS * _amountB * 1000) <=
                                      (_fillAmountB * _amountS * 1001);
        expectedAutomaticValid = expectedAutomaticValid &&
      ...
      SECTION("Random") {
        for (unsigned int j = 0; j < numIterations; j++) {
          checkFillRateChecked(getRandomFieldElementAsBigInt(n),
      ...
      if (n == NUM_BITS_AMOUNT) {
        ...
        SECTION("Exhaustive checks against a single value") {
          ...
          for (unsigned int fillS = 0; fillS < targetFillS * 2; fillS++) {
            bool expectedValid =
                fillS <= targetFillS + targetFillS / 1000 && fillS > 0;
            checkFillRateChecked(20000, 2000, fillS, targetFillB,
                                 expectedValid ? Expected::Valid
                                               : Expected::Invalid);
          }
          ...
          for (unsigned int fillB = 0; fillB < targetFillB * 2; fillB++) {
            bool expectedValid =
                fillB > targetFillB - targetFillB / 1000 && fillB > 0;
            checkFillRateChecked(20000, 2000, targetFillS, fillB,
                                 expectedValid ? Expected::Valid
                                               : Expected::Invalid);
          }
        }
...
TEST_CASE("RequireFillLimit", "[RequireFillLimitGadget]") {
  ...
  const OrderState _orderState = {order, account, balanceLeafS, balanceLeafB,
                                  storageLeaf};
  ...
  enum class Expected { Valid, Invalid, Automatic };
  auto checkFillLimitChecked =
      [_orderState](const FieldT &_amountS, const FieldT &_amountB, bool _fillAmountBorS,
                    unsigned int _storageID, unsigned int _leafStorageID,
                    const FieldT &_filled, const FieldT &_fillAmountS,
                    const FieldT &_fillAmountB,
                    Expected expected = Expected::Automatic) {
        protoboard<FieldT> pb;

        OrderState orderState = setOrderState(
            _orderState, _storageID, _amountS, _amountB, _fillAmountBorS,
            getMaxFieldElement(NUM_BITS_AMOUNT), _filled, _leafStorageID);
  ...
  SECTION("order: max/max, fill: max/max") {
    checkFillLimitChecked(maxAmount, maxAmount, true, 0, 0, 0, 1, 1,
                          Expected::Valid);
  }

  SECTION("order: 1/1, fill: max/max") {
    checkFillLimitChecked(1, 1, true, 0, 0, 0, maxAmount, maxAmount,
                          Expected::Invalid);
  }
  ...
  SECTION("order: max/1, fill: max/1") {
    checkFillLimitChecked(maxAmount, 1, true, 0, 0, 0, maxAmount, 1,
                          Expected::Valid);
  }

  SECTION("order: 1/max, fill: 1/max") {
    checkFillLimitChecked(1, maxAmount, true, 0, 0, 0, 1, maxAmount,
                          Expected::Valid);
  }

  SECTION("reuse slot") {
    unsigned int storageID = rand() % NUM_BITS_STORAGE_ADDRESS;
    checkFillLimitChecked(2, 2, true, storageID, storageID, 1, 1, 1,
                          Expected::Valid);
  }

  SECTION("overwrite slot") {
    unsigned int storageID = rand() % NUM_BITS_STORAGE_ADDRESS;
    checkFillLimitChecked(maxAmount, maxAmount, true,
                          storageID + numStorageSlots, storageID, maxAmount,
  ...
  SECTION("Fill limits") {
    ...
    for (unsigned int filled = 0; filled < amountB * 2; filled += 10) {
      for (unsigned int fillB = 0; fillB < amountB * 2; fillB += 10) {
        ...
        checkFillLimitChecked(
            amountS, amountB, true, storageID, storageID, filled, fillB * 9,
            fillB, expectedValid ? Expected::Valid : Expected::Invalid);
      }
    }
    ...
    for (unsigned int filled = 0; filled < amountS * 2; filled += 100) {
      for (unsigned int fillS = 0; fillS < amountS * 2; fillS += 100) {
        ...
        checkFillLimitChecked(
            amountS, amountB, false, storageID, storageID, filled, fillS,
            fillS / 9, expectedValid ? Expected::Valid : Expected::Invalid);
      }
    }
  }
  ...
  SECTION("Random") {
    for (unsigned int i = 0; i < numIterations; i++) {
      ...
      unsigned int storageID =
          (rand() % 2) == 0 ? leafStorageID : leafStorageID + NUM_STORAGE_SLOTS;
      checkFillLimitChecked(getRandomFieldElement(n), getRandomFieldElement(n),
                            fillAmountBorS, storageID, leafStorageID,
...
TradeHistory getTradeHistory(const OrderState &orderState) {
  FieldT filled =
      lt(orderState.storageLeaf.storageID, orderState.order.storageID)
          ? 0
          : orderState.storageLeaf.data;
  return {filled};
}
...
Fill getMaxFillAmounts(const OrderState &orderState) {
  ...
  if (orderState.order.fillAmountBorS == FieldT::one()) {
    FieldT filled = lt(orderState.order.amountB, tradeHistory.filled)
                        ? orderState.order.amountB
                        : tradeHistory.filled;
    FieldT remainingB = orderState.order.amountB - filled;
    remainingS =
        muldiv(remainingB, orderState.order.amountS, orderState.order.amountB);
  } else {
    FieldT filled = lt(orderState.order.amountS, tradeHistory.filled)
                        ? orderState.order.amountS
                        : tradeHistory.filled;
    remainingS = orderState.order.amountS - filled;
  }
  FieldT fillAmountS = lt(orderState.balanceLeafS.balance, remainingS)
                           ? orderState.balanceLeafS.balance
                           : remainingS;
  FieldT fillAmountB =
      muldiv(fillAmountS, orderState.order.amountB, orderState.order.amountS);
  return {fillAmountS, fillAmountB};
}

bool match(const Order &takerOrder, Fill &takerFill, const Order &makerOrder,
           Fill &makerFill) {
...
bool checkFillRate(const FieldT &amountS, const FieldT &amountB,
                   const FieldT &fillAmountS, const FieldT &fillAmountB) {
  return lte(fillAmountS * amountB * FieldT(1000),
             fillAmountB * amountS * FieldT(1001));
}
...
Settlement settle(const FieldT &timestamp, const OrderState &orderStateA,
...
TEST_CASE("OrderMatching", "[OrderMatchingGadget]") {
  enum class ExpectedSatisfied { Satisfied, NotSatisfied, Automatic };
  enum class ExpectedValid { Valid, Invalid, Automatic };
  enum class ExpectedFill { Manual, Automatic };
  auto orderMatchingChecked = [](const FieldT &_exchange,
    ...
    FieldT limitA = orderStateA.order.fillAmountBorS == FieldT::one()
                        ? orderStateA.order.amountB
                        : orderStateA.order.amountS;
    expectedSatisfiedValue = lte(pb.val(storageA.getData()), limitA);
    FieldT limitB = orderStateB.order.fillAmountBorS == FieldT::one()
                        ? orderStateB.order.amountB
                        : orderStateB.order.amountS;
    expectedSatisfiedValue =
        expectedSatisfiedValue && lte(pb.val(storageB.getData()), limitB);

    if (expectedSatisfied != ExpectedSatisfied::Automatic) {
      expectedSatisfiedValue =
          (expectedSatisfied == ExpectedSatisfied::Satisfied) ? true : false;
    }

    Simulator::Settlement settlement =
        Simulator::settle(_timestamp, orderStateA, orderStateB);
    bool expectedValidValue = settlement.valid;
    if (expectedValid != ExpectedValid::Automatic) {
      expectedValidValue =
          (expectedValid == ExpectedValid::Valid) ? true : false;
    }
    ...
    VariableT expectedFillS_A =
        make_variable(pb, roundToFloatValue(_expectedFillS_A, Float24Encoding),
                      "expectedFillS_A");
    VariableT expectedFillS_B =
        make_variable(pb, roundToFloatValue(_expectedFillS_B, Float24Encoding),
                      "expectedFillS_B");
    ...
    REQUIRE(pb.is_satisfied() ==
            (expectedSatisfiedValue && expectedValidValue));
    if (expectedSatisfiedValue && expectedValidValue) {
      auto tradeHistory_A = Simulator::getTradeHistory(orderStateA);
      REQUIRE((pb.val(orderMatching.getFilledAfter_A()) ==
               (tradeHistory_A.filled + (orderStateA.order.fillAmountBorS == 1
                                             ? pb.val(expectedFillS_B)
                                             : pb.val(expectedFillS_A)))));
      auto tradeHistory_B = Simulator::getTradeHistory(orderStateB);
      REQUIRE((pb.val(orderMatching.getFilledAfter_B()) ==
               (tradeHistory_B.filled + (orderStateB.order.fillAmountBorS == 1
                                             ? pb.val(expectedFillS_A)
                                             : pb.val(expectedFillS_B)))));
    }
  };
  ...
  const OrderState orderStateA = {A_order, A_account, A_balanceLeafS,
                                  A_balanceLeafB, A_storageLeaf};
  const FieldT expectFillS_A(
      fromFloat(tx.spotTrade.fillS_A.as_ulong(), Float24Encoding)
          .to_string()
          .c_str());
  ...
  const OrderState orderStateB = {B_order, B_account, B_balanceLeafS,
                                  B_balanceLeafB, B_storageLeaf};
  const FieldT expectFillS_B(
      fromFloat(tx.spotTrade.fillS_B.as_ulong(), Float24Encoding)
          .to_string()
          .c_str());
  ...
  SECTION("Valid order match") {
    orderMatchingChecked(exchange, timestamp, orderStateA, orderStateB,
  ...
  SECTION("orderA.tokenS != orderB.tokenB") {
    ...
    orderMatchingChecked(exchange, timestamp, orderStateA_mod, orderStateB,
  ...
  SECTION("orderA.amountS/B = maxAmount, orderB.amountS/B = maxAmount") {
    for (unsigned int i = 0; i < 4; i++) {
      ...
      OrderState orderStateA_mod =
          setOrderState(orderStateA, A_storageID, maxAmount, maxAmount, fillAmountBorS_A,
                        maxAmount, 0, A_storageID);
      OrderState orderStateB_mod =
          setOrderState(orderStateB, B_storageID, maxAmount, maxAmount, fillAmountBorS_B,
  ...
  SECTION("orderA.amountS/B = maxAmount, orderB.amountS/B = 1") {
    for (unsigned int i = 0; i < 4; i++) {
      ...
      OrderState orderStateB_mod = setOrderState(
          orderStateB, B_storageID, 1, 1, fillAmountBorS_B, maxAmount, 0, B_storageID);
      orderMatchingChecked(exchange, timestamp, orderStateA_mod,
                           orderStateB_mod, ExpectedSatisfied::Satisfied,
                           ExpectedValid::Valid, ExpectedFill::Manual, 1, 1);
    }
  }
  ...
  SECTION("orderA.amountS/B = 1, orderB.amountS/B = maxAmount") {
    for (unsigned int i = 0; i < 4; i++) {
      ...
      OrderState orderStateA_mod = setOrderState(
          orderStateA, A_storageID, 1, 1, fillAmountBorS_A, maxAmount, 0, A_storageID);
      OrderState orderStateB_mod =
          setOrderState(orderStateB, B_storageID, maxAmount, maxAmount, fillAmountBorS_B,
                        maxAmount, 0, B_storageID);
      orderMatchingChecked(exchange, timestamp, orderStateA_mod,
  ...
  SECTION("orderA.amountS/B = maxAmount/1, orderB.amountS/B = maxAmount") {
    for (unsigned int i = 0; i < 4; i++) {
      ...
      bool fillAmountBorS_B = i / 2;
      OrderState orderStateA_mod =
          setOrderState(orderStateA, A_storageID, maxAmount, 1, fillAmountBorS_A, maxAmount,
                        0, A_storageID);
      OrderState orderStateB_mod =
          setOrderState(orderStateB, B_storageID, maxAmount, maxAmount, fillAmountBorS_B,
                        maxAmount, 0, B_storageID);
      orderMatchingChecked(exchange, timestamp, orderStateA_mod,
                           orderStateB_mod, ExpectedSatisfied::Satisfied,
                           ExpectedValid::Valid, ExpectedFill::Manual,
                           fillAmountBorS_A ? 1 : maxAmount, fillAmountBorS_A ? 1 : maxAmount);
    }
  }
  ...
  SECTION("orderA.amountS/B = 1/maxAmount, orderB.amountS/B = maxAmount") {
    for (unsigned int i = 0; i < 4; i++) {
      ...
      bool fillAmountBorS_B = i / 2;
      OrderState orderStateA_mod =
          setOrderState(orderStateA, A_storageID, 1, maxAmount, fillAmountBorS_A, maxAmount,
                        0, A_storageID);
      OrderState orderStateB_mod =
          setOrderState(orderStateB, B_storageID, maxAmount, maxAmount, fillAmountBorS_B,
                        maxAmount, 0, B_storageID);
      orderMatchingChecked(exchange, timestamp, orderStateA_mod,
                           orderStateB_mod, ExpectedSatisfied::Satisfied,
                           ExpectedValid::Invalid, ExpectedFill::Manual,
                           fillAmountBorS_A ? maxAmountFill : 1, maxAmountFill);
    }
  }

  SECTION(
      "orderA.amountS/B = [1/maxAmount], orderB.amountS/B = [1/maxAmount]") {
    for (unsigned int c = 0; c < pow(2, 6); c++) {
      ...
      for (unsigned int i = 0; i < 4; i++) {
        ...
        bool fillAmountBorS_B = i / 2;
        OrderState orderStateA_mod =
            setOrderState(orderStateA, A_storageID, A_amountS, A_amountB, fillAmountBorS_A,
                          A_balance, 0, A_storageID);
        OrderState orderStateB_mod =
            setOrderState(orderStateB, B_storageID, B_amountS, B_amountB, fillAmountBorS_B,
                          B_balance, 0, B_storageID);
        orderMatchingChecked(exchange, timestamp, orderStateA_mod,
                             orderStateB_mod, ExpectedSatisfied::Satisfied,
                             ExpectedValid::Automatic, ExpectedFill::Automatic);
      }
    }
  }

  SECTION(
      "storageID == tradeHistory.storageID (filled amount > amountS/amountB)") {
    for (unsigned int i = 0; i < 2; i++) {
      bool fillAmountBorS_A = i % 2;
      OrderState orderStateA_mod =
          setOrderState(orderStateA, A_storageID, 1, 1, fillAmountBorS_A, maxAmount,
                        maxAmount, A_storageID);
      OrderState orderStateB_mod = setOrderState(orderStateB, B_storageID, 1, 1,
                                                 false, 1, 0, B_storageID);
      orderMatchingChecked(exchange, timestamp, orderStateA_mod,
                           orderStateB_mod, ExpectedSatisfied::Satisfied,
                           ExpectedValid::Invalid, ExpectedFill::Manual, 0, 0);
    }
  }
  ...
  SECTION("storageID > tradeHistory.storageID") {
    for (unsigned int i = 0; i < 4; i++) {
      ...
      bool fillAmountBorS_B = i / 2;
      OrderState orderStateA_mod =
          setOrderState(orderStateA, A_storageID + numStorageSlots, maxAmount,
                        maxAmount, fillAmountBorS_A, maxAmount, maxAmount, A_storageID);
      OrderState orderStateB_mod =
          setOrderState(orderStateB, B_storageID + numStorageSlots, maxAmount,
                        maxAmount, fillAmountBorS_B, maxAmount, maxAmount, B_storageID);
      orderMatchingChecked(exchange, timestamp, orderStateA_mod,
                           orderStateB_mod, ExpectedSatisfied::Satisfied,
                           ExpectedValid::Valid, ExpectedFill::Manual,
                           maxAmount, maxAmount);
  ...
  SECTION("Filled (buy)") {
    for (unsigned int i = 0; i < 400; i += 2) {
      OrderState orderStateA_mod = setOrderState(
          orderStateA, A_storageID, 400, 200, true, maxAmount, i, A_storageID);
      OrderState orderStateB_mod =
  ...
  SECTION("Filled (sell)") {
    for (unsigned int i = 0; i < 400; i += 2) {
      OrderState orderStateA_mod = setOrderState(
          orderStateA, A_storageID, 200, 100, false, maxAmount, i, A_storageID);
      OrderState orderStateB_mod =
          setOrderState(orderStateB, B_storageID, 400, 400, bool(rand() % 2),
                        maxAmount, 0, B_storageID);
  ...
  SECTION("Random") {
    for (unsigned int i = 0; i < 128; i++) {
      ...
      OrderState orderStateB_mod = setOrderState(
          orderStateB, B_storageID, getRandomFieldElement(NUM_BITS_AMOUNT),
          getRandomFieldElement(NUM_BITS_AMOUNT), bool(rand() % 2),
          getRandomFieldElement(NUM_BITS_AMOUNT),
          getRandomFieldElement(NUM_BITS_AMOUNT), B_storageID);
      orderMatchingChecked(exchange, timestamp, orderStateA_mod,
                           orderStateB_mod, ExpectedSatisfied::Automatic,
                           ExpectedValid::Automatic, ExpectedFill::Automatic);
    }
  }
  ...
  SECTION("Regression Case 1, expectedFill as maxFill causes order mismatch") {
    OrderState orderStateA_mod = setOrderState(
        orderStateA, A_storageID, FieldT("74437628000000000000000"),
        FieldT("63821764000000000000000"), false,
        FieldT("89266748000000000000000"), 0, A_storageID);
    OrderState orderStateB_mod = setOrderState(
        orderStateB, B_storageID, FieldT("42719051000000000000000"),
        FieldT("2883176000000000000000"), true,
        FieldT("42719180000000000000000"), 0, B_storageID);
    FieldT expectedFillS_A("2883170000000000000000");
    FieldT expectedFillS_B("42719000000000000000000");
    orderMatchingChecked(exchange, timestamp, orderStateA_mod, orderStateB_mod,
                         ExpectedSatisfied::Satisfied, ExpectedValid::Valid,
                         ExpectedFill::Manual, expectedFillS_A,
                         expectedFillS_B);
  }
}
