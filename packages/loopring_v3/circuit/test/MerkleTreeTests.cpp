...
AccountState createAccountState(//
      ProtoboardT &pb, const AccountLeaf &state) {
  ...
  accountState.balancesRoot =
      make_variable(pb, state.balancesRoot, ".balancesRoot");
  return accountState;
}

BalanceState createBalanceState(//
      ProtoboardT &pb, const BalanceLeaf &state) {
...
StorageState createStorageState(//
      ProtoboardT &pb, const StorageLeaf &state) {
...
TEST_CASE("UpdateAccount", "[UpdateAccountGadget]") {
  ...
  auto updateAccountChecked = [](const AccountUpdate &accountUpdate,
                                 bool expectedSatisfied,
                                 bool expectedRootAfterCorrect = true) {
    ...
    UpdateAccountGadget updateAccount(pb, rootBefore, address, stateBefore,
                                      stateAfter, "updateAccount");
    ...
    if (expectedSatisfied) {
      REQUIRE((pb.val(updateAccount.result()) == accountUpdate.rootAfter) ==
              expectedRootAfterCorrect);
    }
  };
...
TEST_CASE("UpdateBalance", "[UpdateBalanceGadget]") {
  ...
  auto updateBalanceChecked = [](const BalanceUpdate &balanceUpdate,
                                 bool expectedSatisfied,
                                 bool expectedRootAfterCorrect = true) {
    protoboard<FieldT> pb;
    ...
    UpdateBalanceGadget updateBalance(pb, rootBefore, address, stateBefore,
                                      stateAfter, "updateBalance");
    updateBalance.generate_r1cs_constraints();
    updateBalance.generate_r1cs_witness(balanceUpdate);

    REQUIRE(pb.is_satisfied() == expectedSatisfied);
    if (expectedSatisfied) {
      REQUIRE((pb.val(updateBalance.result()) == balanceUpdate.rootAfter) ==
              expectedRootAfterCorrect);
    }
  };
...
TEST_CASE("UpdateStorage", "[UpdateStorageGadget]") {
  ...
  auto updateStorageChecked = [](const StorageUpdate &storageUpdate,
                                 bool expectedSatisfied,
                                 bool expectedRootAfterCorrect = true) {
    protoboard<FieldT> pb;
    ...
    VariableArrayT address =
        make_var_array(pb, NUM_BITS_STORAGE_ADDRESS, ".address");
    ...
    UpdateStorageGadget updateStorage(
        pb, rootBefore, subArray(address, 0, NUM_BITS_STORAGE_ADDRESS),
        stateBefore, stateAfter, "updateStorage");
    updateStorage.generate_r1cs_constraints();
    updateStorage.generate_r1cs_witness(storageUpdate);

    REQUIRE(pb.is_satisfied() == expectedSatisfied);
    if (expectedSatisfied) {
      REQUIRE((pb.val(updateStorage.result()) == storageUpdate.rootAfter) ==
              expectedRootAfterCorrect);
    }
  };
