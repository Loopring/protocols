  ...
  for (unsigned int n = 1; n < maxNumInputs; n++) {
    DYNAMIC_SECTION("Length: " << n) {
      for (unsigned int j = 0; j < numIterations; j++) {
        ...
        ArrayTernaryGadget arrayTernaryGadget(pb, b, A, B,
                                              "arrayTernaryGadget");
        ...
        REQUIRE(pb.is_satisfied());
        for (unsigned int i = 0; i < n; i++) {
          REQUIRE((pb.val(arrayTernaryGadget.result()[i]) ==
                   (toggle ? pb.val(A[i]) : pb.val(B[i]))));
        }

        // Flip a bit
        unsigned int randomBit = rand() % n;
        pb.val(arrayTernaryGadget.result()[randomBit]) =
            FieldT::one() - pb.val(arrayTernaryGadget.result()[randomBit]);
  ...
  for (unsigned int n = 2; n < maxNumInputs; n++) {
    DYNAMIC_SECTION("Num inputs: " << n) {
      for (unsigned int j = 0; j < numIterations; j++) {
        ...
        for (unsigned int i = 0; i < n; i++) {
          // Bias to 1s the more inputs there are, otherwise the result is
          // almost always 0
          inputs.emplace_back(
              make_variable(pb, (rand() % n == 0) ? 0 : 1, "i"));
        }
  ...
  for (unsigned int n = 2; n < maxNumInputs; n++) {
    DYNAMIC_SECTION("Num inputs: " << n) {
      for (unsigned int j = 0; j < numIterations; j++) {
        ...
        std::vector<VariableT> inputs;
        for (unsigned int i = 0; i < n; i++) {
          // Bias to 0s the more inputs there are, otherwise the result is
          // almost always 1
          inputs.emplace_back(
              make_variable(pb, (rand() % n == 0) ? 1 : 0, "i"));
        }
  ...
  for (unsigned int n = 1; n < maxNumInputs; n++) {
    DYNAMIC_SECTION("Num inputs: " << n) {
      for (unsigned int j = 0; j < numIterations; j++) {
        ...
        VariableArrayT expectedResult =
            make_var_array(pb, n, ".expectedResult");
        for (unsigned int i = 0; i < n; i++) {
          pb.val(expectedResult[i]) = pb.val(A[i]).as_bigint().as_ulong() ^
                                      pb.val(B[i]).as_bigint().as_ulong();
          REQUIRE((pb.val(xorArrayGadget.result()[i]) ==
                   pb.val(expectedResult[i])));
        }
...
TEST_CASE("Equal", "[EqualGadget]") {
  ...
  SECTION("0 == 0") { equalChecked(0, 0); }

  SECTION("max == max") { equalChecked(max, max); }
...
TEST_CASE("RequireEqual", "[RequireEqualGadget]") {
  ...
  SECTION("0 == 0") { requireEqualChecked(0, 0); }

  SECTION("max == max") { requireEqualChecked(max, max); }
...
TEST_CASE("RequireZeroAorB", "[RequireZeroAorBGadget]") {
  ...
  auto requireZeroAorBChecked = [](const FieldT& _A, const FieldT& _B) {
    ...
    RequireZeroAorBGadget requireZeroAorBGadget(pb, a, b,
  ...
  SECTION("0 || 0") { requireZeroAorBChecked(0, 0); }
  ...
  for (unsigned int n = 1; n <= maxLength; n++) {
    DYNAMIC_SECTION("Bit-length: " << n) {
      auto requireNotEqualChecked = [n](const FieldT& _A, const FieldT& _B) {
        ...
        RequireNotEqualGadget requireNotEqualGadget(pb, a, b,
      ...
      SECTION("0 != 0") { requireNotEqualChecked(0, 0); }

      SECTION("max != max") { requireNotEqualChecked(max, max); }
      ...
      SECTION("0 != max") { requireNotEqualChecked(0, max); }

      SECTION("max != 0") { requireNotEqualChecked(max, 0); }

      SECTION("Random") {
        for (unsigned int j = 0; j < numIterations; j++) {
          requireNotEqualChecked(getRandomFieldElement(n),
...
TEST_CASE("IfThenRequire", "[IfThenRequireGadget]") {
  ...
  SECTION("if false then false") { ifThenRequireChecked(0, 0); }

  SECTION("if false then true") { ifThenRequireChecked(0, 1); }

  SECTION("if true then false") { ifThenRequireChecked(1, 0); }

  SECTION("if false then true") { ifThenRequireChecked(1, 1); }
}

TEST_CASE("IfThenRequireEqual", "[IfThenRequireEqualGadget]") {
  auto ifThenRequireEqualChecked = [](const BigInt& _C, const BigInt& _A,
                                      const BigInt& _B) {
    protoboard<FieldT> pb;
    ...
    IfThenRequireEqualGadget ifThenRequireEqualGadget(
        pb, c, a, b, "ifThenRequireEqualGadget");
  ...
  SECTION("if false then (A != B)") { ifThenRequireEqualChecked(0, A, B); }

  SECTION("if false then (A == B)") { ifThenRequireEqualChecked(0, A, A); }

  SECTION("if true then (A != B)") { ifThenRequireEqualChecked(1, B, A); }

  SECTION("if true then (A == B)") { ifThenRequireEqualChecked(1, B, B); }
}

TEST_CASE("IfThenRequireNotEqual", "[IfThenRequireNotEqualGadget]") {
  auto ifThenRequireNotEqualGadget = [](const BigInt& _C, const BigInt& _A,
                                        const BigInt& _B) {
    protoboard<FieldT> pb;
    ...
    IfThenRequireNotEqualGadget ifThenRequireNotEqualGadget(
        pb, c, a, b, "ifThenRequireNotEqualGadget");
  ...
  SECTION("if false then (A != B)") { ifThenRequireNotEqualGadget(0, A, B); }

  SECTION("if false then (A == B)") { ifThenRequireNotEqualGadget(0, A, A); }

  SECTION("if true then (A != B)") { ifThenRequireNotEqualGadget(1, B, A); }

  SECTION("if true then (A == B)") { ifThenRequireNotEqualGadget(1, B, B); }
}
...
TEST_CASE("Min", "[MinGadget]") {
  ...
  for (unsigned int n = 1; n <= maxLength; n++) {
    DYNAMIC_SECTION("Bit-length: " << n) {
      ...
      SECTION("min(0, 0)") { minChecked(0, 0); }

      SECTION("min(1, 0)") { minChecked(1, 0); }

      SECTION("min(0, 1)") { minChecked(0, 1); }

      SECTION("min(max, max)") { minChecked(max, max); }

      SECTION("min(max - 1, max)") { minChecked(max - 1, max); }

      SECTION("min(max, max - 1)") { minChecked(max, max - 1); }

      SECTION("min(0, max)") { minChecked(0, max); }

      SECTION("min(max, 0)") { minChecked(max, 0); }

      SECTION("Random") {
        for (unsigned int j = 0; j < numIterations; j++) {
          minChecked(getRandomFieldElementAsBigInt(n),
...
TEST_CASE("Max", "[MaxGadget]") {
  ...
  for (unsigned int n = 1; n <= maxLength; n++) {
    DYNAMIC_SECTION("Bit-length: " << n) {
      ...
      SECTION("max(0, 0)") { maxChecked(0, 0); }

      SECTION("max(1, 0)") { maxChecked(1, 0); }

      SECTION("max(0, 1)") { maxChecked(0, 1); }

      SECTION("max(max, max)") { maxChecked(max, max); }

      SECTION("max(max - 1, max)") { maxChecked(max - 1, max); }

      SECTION("max(max, max - 1)") { maxChecked(max, max - 1); }

      SECTION("max(0, max)") { maxChecked(0, max); }

      SECTION("max(max, 0)") { maxChecked(max, 0); }

      SECTION("Random") {
        for (unsigned int j = 0; j < numIterations; j++) {
          maxChecked(getRandomFieldElementAsBigInt(n),
                     getRandomFieldElementAsBigInt(n));
        }
      }
    }
  ...
  for (unsigned int n = 1; n <= maxLength; n++) {
    DYNAMIC_SECTION("Bit-length: " << n) {
      auto leqChecked = [n](const BigInt& _A, const BigInt& _B) {
        ...
        REQUIRE(pb.is_satisfied());
        REQUIRE((pb.val(leqGadget.lt()) ==
                 (expectedLt ? FieldT::one() : FieldT::zero())));
        REQUIRE((pb.val(leqGadget.leq()) ==
                 (expectedLeq ? FieldT::one() : FieldT::zero())));
        REQUIRE((pb.val(leqGadget.eq()) ==
                 (expectedEq ? FieldT::one() : FieldT::zero())));
        REQUIRE((pb.val(leqGadget.gte()) ==
                 (expectedGte ? FieldT::one() : FieldT::zero())));
        REQUIRE((pb.val(leqGadget.gt()) ==
                 (expectedGt ? FieldT::one() : FieldT::zero())));
      };

      BigInt max = getMaxFieldElementAsBigInt(n);

      SECTION("0 <(=) 0") { leqChecked(0, 0); }

      SECTION("0 <(=) 1") { leqChecked(0, 1); }

      SECTION("max <(=) max") { leqChecked(max, max); }

      SECTION("max - 1 <(=) max") { leqChecked(max - 1, max); }

      SECTION("max <(=) max - 1") { leqChecked(max, max - 1); }

      SECTION("0 <(=) max") { leqChecked(0, max); }

      SECTION("max <(=) 0") { leqChecked(max, 0); }

      SECTION("Random") {
        for (unsigned int j = 0; j < numIterations; j++) {
          leqChecked(getRandomFieldElementAsBigInt(n),
                     getRandomFieldElementAsBigInt(n));
        }
      }
    }
...
TEST_CASE("RequireLeq", "[RequireLeqGadget]") {
  ...
  for (unsigned int n = 1; n <= maxLength; n++) {
    DYNAMIC_SECTION("Bit-length: " << n) {
      ...
      SECTION("0 <= 0") { requireLeqChecked(0, 0); }

      SECTION("0 <= 1") { requireLeqChecked(0, 1); }

      SECTION("max <= max") { requireLeqChecked(max, max); }

      SECTION("max - 1 <= max") { requireLeqChecked(max - 1, max); }

      SECTION("max <= max - 1") { requireLeqChecked(max, max - 1); }

      SECTION("0 <= max") { requireLeqChecked(0, max); }

      SECTION("max <= 0") { requireLeqChecked(max, 0); }

      SECTION("Random") {
        for (unsigned int j = 0; j < numIterations; j++) {
          requireLeqChecked(getRandomFieldElementAsBigInt(n),
                            getRandomFieldElementAsBigInt(n));
        }
      }
    }
...
TEST_CASE("RequireLt", "[RequireLtGadget]") {
  ...
  for (unsigned int n = 1; n <= maxLength; n++) {
    DYNAMIC_SECTION("Bit-length: " << n) {
      ...
      SECTION("0 < 0") { requireLtChecked(0, 0); }

      SECTION("max < max") { requireLtChecked(max, max); }

      SECTION("0 < max") { requireLtChecked(0, max); }

      SECTION("max < 0") { requireLtChecked(max, 0); }

      SECTION("Random") {
        for (unsigned int j = 0; j < numIterations; j++) {
          requireLtChecked(getRandomFieldElementAsBigInt(n),
                           getRandomFieldElementAsBigInt(n));
        }
      }
    }
...
TEST_CASE("MulDiv", "[MulDivGadget]") {
  ...
  for (unsigned int n = 1; n <= maxLength; n++) {
    DYNAMIC_SECTION("Bit-length: " << n) {
      auto mulDivChecked = [n](const BigInt& _value, const BigInt& _numerator,
                               const BigInt& _denominator,
                               bool expectedSatisfied,
                               bool bModifyRemainder = false) {
        protoboard<FieldT> pb;

        pb_variable<FieldT> value =
            make_variable(pb, toFieldElement(_value), "value");
        pb_variable<FieldT> numerator =
            make_variable(pb, toFieldElement(_numerator), "numerator");
        pb_variable<FieldT> denominator =
            make_variable(pb, toFieldElement(_denominator), "denominator");

        Constants constants(pb, "constants");
        MulDivGadget mulDivGadget(pb, constants, value, numerator, denominator,
                                  n, n, n, "mulDivGadget");
        ...
        if (expectedSatisfied) {
          ...
          REQUIRE((pb.val(mulDivGadget.getRemainder()) ==
                   toFieldElement(remainder)));
          REQUIRE(
              (pb.val(mulDivGadget.getProduct()) == toFieldElement(product)));
        }
      };
      ...
      SECTION("Divide by zero") {
        for (unsigned int j = 0; j < numIterations; j++) {
          mulDivChecked(getRandomFieldElementAsBigInt(n),
                        getRandomFieldElementAsBigInt(n), 0, false);
        }
      }

      SECTION("0 * 0 / 1 = 0") { mulDivChecked(0, 0, 1, true); }

      SECTION("1 * 1 / 1 = 1") { mulDivChecked(1, 1, 1, true); }

      SECTION("max * max / max = max") { mulDivChecked(max, max, max, true); }

      SECTION("max * max / 1 = max * max") { mulDivChecked(max, max, 1, true); }

      SECTION("remainder >= C") { mulDivChecked(max, max, max, false, true); }
      ...
      SECTION("Random") {
        for (unsigned int j = 0; j < numIterations; j++) {
          ...
          mulDivChecked(getRandomFieldElementAsBigInt(n),
                        getRandomFieldElementAsBigInt(n), denominator, true);
        }
      }
    }
  }
...
TEST_CASE("Add", "[AddGadget]") {
  ...
  for (unsigned int n = 1; n <= maxLength; n++) {
    DYNAMIC_SECTION("Bit-length: " << n) {
      ...
      SECTION("0 + 0") { addChecked(0, 0); }

      SECTION("0 + 1") { addChecked(0, 1); }

      SECTION("max + 0") { addChecked(max, 0); }

      SECTION("halfMax + halfMax + 1") { addChecked(halfMax, halfMax + 1); }

      SECTION("max + 1 (overflow)") { addChecked(max, 1); }

      SECTION("max + max (overflow)") { addChecked(max, max); }
...
TEST_CASE("Sub", "[SubGadget]") {
  ...
  for (unsigned int n = 1; n <= maxLength; n++) {
    DYNAMIC_SECTION("Bit-length: " << n) {
      ...
      SECTION("0 - 0") { subChecked(0, 0); }

      SECTION("0 - 1") { subChecked(0, 1); }

      SECTION("1 - 0") { subChecked(1, 0); }

      SECTION("max - 0") { subChecked(max, 0); }

      SECTION("max - 1") { subChecked(max, 1); }

      SECTION("0 - max") { subChecked(0, max); }

      SECTION("max - max") { subChecked(max, max); }
...
TEST_CASE("transfer", "[TransferGadget]") {
  auto transferChecked = [](const FieldT& _from, const FieldT& _to,
                            const FieldT& _amount, bool expectedSatisfied,
                            const FieldT& expectedFromAfter = 0,
                            const FieldT& expectedToAfter = 0) {
    protoboard<FieldT> pb;
  ...
  SECTION("(0, 0) -+ 0") { transferChecked(0, 0, 0, true, 0, 0); }

  SECTION("(1, 0) -+ 1") { transferChecked(1, 0, 1, true, 0, 1); }

  SECTION("(max, 0) -+ 0") { transferChecked(max, 0, 0, true, max, 0); }

  SECTION("(max, 0) -+ max") { transferChecked(max, 0, max, true, 0, max); }
  ...
  SECTION("(0, 0) -+ 1 (underflow)") { transferChecked(0, 0, 1, false); }

  SECTION("(0, 0) -+ max (underflow)") { transferChecked(0, 0, max, false); }
  ...
  for (unsigned int n = 1; n <= maxLength; n++) {
    DYNAMIC_SECTION("Bit-length: " << n) {
      auto rangeLimitChecked = [n](const FieldT& v, bool expectedSatisfied) {
        ...
        libsnark::dual_variable_gadget<FieldT> rangeLimitedValue(
            pb, value, n, "dual_variable_gadget");
        ...
        if (expectedSatisfied) {
          ...
          REQUIRE(compareBits(rangeLimitedValue.bits.get_bits(pb),
                              toBits(pb.val(value), n)));
        }
      };

      SECTION("0") { rangeLimitChecked(0, true); }

      SECTION("1") { rangeLimitChecked(0, true); }

      SECTION("max") { rangeLimitChecked(getMaxFieldElement(n), true); }
...
TEST_CASE("LtField", "[LtFieldGadget]") {
  ...
  auto LtFieldChecked = [](const BigInt& _A, const BigInt& _B) {
    ...
    bool expectedLt = _A < _B;
    REQUIRE(pb.is_satisfied());
    REQUIRE((pb.val(ltFieldGadget.lt()) ==
             (expectedLt ? FieldT::one() : FieldT::zero())));
  };

  BigInt max = getMaxFieldElementAsBigInt(n);

  SECTION("0 < 0") { LtFieldChecked(0, 0); }

  SECTION("0 < 1") { LtFieldChecked(0, 1); }

  SECTION("1 < 0") { LtFieldChecked(1, 0); }

  SECTION("max < max") { LtFieldChecked(max, max); }

  SECTION("max - 1 < max") { LtFieldChecked(max - 1, max); }

  SECTION("max < max - 1") { LtFieldChecked(max, max - 1); }

  SECTION("0 < max") { LtFieldChecked(0, max); }

  SECTION("max < 0") { LtFieldChecked(max, 0); }

  SECTION("Random") {
    for (unsigned int j = 0; j < numIterations; j++) {
      LtFieldChecked(getRandomFieldElementAsBigInt(n),
                     getRandomFieldElementAsBigInt(n));
    }
  }
}
...
TEST_CASE("Selector", "[SelectorGadget]") {
  ...
  auto selectorChecked = [](const FieldT& _type, unsigned int maxBits) {
    ...
    SelectorGadget selectorGadget(pb, constants, type, maxBits,
                                  "selectorGadget");
    ...
    for (unsigned int i = 0; i < maxBits; i++) {
      expectedBits.push_back((FieldT(i) == _type) ? FieldT::one()
                                                  : FieldT::zero());
    ...
    pb.val(selectorGadget.result()[randomBit]) =
        FieldT::one() - pb.val(selectorGadget.result()[randomBit]);
    REQUIRE(pb.is_satisfied() == false);
  };

  SECTION("No bits selected") { selectorChecked(FieldT(6), 5); }
...
TEST_CASE("Select", "[SelectGadget]") {
  ...
  auto selectChecked = [](unsigned int _index, std::vector<FieldT>& _values) {
    ...
    SelectorGadget selectorGadget(pb, constants, index, values.size(),
                                  "selectorGadget");
    selectorGadget.generate_r1cs_constraints();
    selectorGadget.generate_r1cs_witness();

    SelectGadget selectGadget(pb, selectorGadget.result(), values,
                              "selectGadget");
    ...
    unsigned int randomIndex =
        (_index + 1 + rand() % (_values.size() - 1)) % _values.size();
...
TEST_CASE("ArraySelect", "[ArraySelectGadget]") {
  ...
  auto selectArrayChecked = [](unsigned int _index, unsigned int numValues,
    ...
    ArraySelectGadget arraySelectGadget(pb, selectorGadget.result(), values,
                                        "arraySelectGadget");
    ...
    for (unsigned int i = 0; i < varLength; i++) {
      REQUIRE(
          (pb.val(arraySelectGadget.result()[i]) == pb.val(values[_index][i])));
    }

    // Flip a bit
    unsigned int randomBit = rand() % varLength;
    pb.val(arraySelectGadget.result()[randomBit]) =
        FieldT::one() - pb.val(arraySelectGadget.result()[randomBit]);
    REQUIRE(pb.is_satisfied() == false);
  };
