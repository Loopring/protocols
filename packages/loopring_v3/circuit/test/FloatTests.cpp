...
TEST_CASE("RequireAccuracy", "[RequireAccuracyGadget]") {
  ...
  for (unsigned int n = 1; n <= maxLength; n++) {
    DYNAMIC_SECTION("Bit-length: " << n) {
      auto requireAccuracyChecked = [n](const FieldT &_A, const FieldT &_B,
        ...
        RequireAccuracyGadget requireAccuracyGadget(pb, a, b, accuracy, n,
      ...
      SECTION("0, 0") { requireAccuracyChecked(0, 0, true); }

      SECTION("0, 1") { requireAccuracyChecked(0, 1, false); }

      SECTION("0, max") { requireAccuracyChecked(0, max, false); }

      SECTION("max, 0") { requireAccuracyChecked(max, 0, false); }
      ...
      if (n == NUM_BITS_AMOUNT) {
        SECTION("100, 100") { requireAccuracyChecked(100, 100, true); }

        SECTION("101, 100") { requireAccuracyChecked(101, 100, false); }

        SECTION("99, 100") { requireAccuracyChecked(99, 100, true); }

        SECTION("max + 1, max") { requireAccuracyChecked(max + 1, max, false); }

        SECTION("max, 3000") { requireAccuracyChecked(max, 3000, false); }
...
TEST_CASE("Float", "[FloatGadget]") {
  ...
  unsigned int numBitsFloat =
      encoding.numBitsExponent + encoding.numBitsMantissa;
  ...
  for (unsigned int n = 1; n <= maxLength; n++) {
    DYNAMIC_SECTION("Bit-length: " << n) {
      auto floatChecked = [n, encoding, numBitsFloat](const FieldT &_value) {
        ...
        REQUIRE(compareBits(floatGadget.bits().get_bits(pb),
                            toBits(f, numBitsFloat)));
      };

      SECTION("0") { floatChecked(0); }

      SECTION("1") { floatChecked(1); }

      SECTION("max") { floatChecked(getMaxFieldElement(n)); }
...
TEST_CASE("Float+Accuracy", "[FloatGadget+RequireAccuracy]") {
  ...
  std::vector<FieldT> worstAccuracyValues = {
      FieldT("20499999999999999999999999999"),
      FieldT("52428999999999999999999999999")};
  for (unsigned int e = 0; e < encodings.size(); e++) {
    DYNAMIC_SECTION("Encoding: " << encodings[e].numBitsExponent +
                                        encodings[e].numBitsMantissa) {
      ...
      RequireAccuracyGadget requireAccuracyGadget(pb, rValue, value, accuracy,
                                                  n, "requireAccuracyGadget");
      requireAccuracyGadget.generate_r1cs_constraints();
