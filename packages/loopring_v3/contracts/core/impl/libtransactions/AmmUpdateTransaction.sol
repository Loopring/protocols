...
library AmmUpdateTransaction {
    ...
    bytes32 public constant AMMUPDATE_TYPEHASH =
        keccak256(
    ...
    function process(
        ...
        AmmUpdateAuxiliaryData memory auxData = abi.decode(
            auxiliaryData,
            (AmmUpdateAuxiliaryData)
        );
    ...
    function readTx(
        ...
        require(
            data.toUint8Unsafe(_offset) ==
                uint8(ExchangeData.TransactionType.AMM_UPDATE),
            "INVALID_TX_TYPE"
        );
        _offset += 1;
