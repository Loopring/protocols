...
abstract contract IDelayedTransaction {
    ...
    function transact(address to, bytes calldata data) external payable virtual;

    /// @dev Executes a pending transaction.
    /// @param transactionId The id of the pending transaction.
    function executeTransaction(uint transactionId) external virtual;

    /// @dev Cancels a pending transaction.
    /// @param transactionId The id of the pending transaction.
    function cancelTransaction(uint transactionId) external virtual;

    /// @dev Cancels all pending transactions.
    function cancelAllTransactions() external virtual;
    ...
    function getNumPendingTransactions() external view virtual returns (uint);

    /// @dev Gets the number of functions that have a delay.
    /// @return The number of delayed functions.
    function getNumDelayedFunctions() external view virtual returns (uint);
}
