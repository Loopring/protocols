...
contract FastWithdrawalLiquidityProvider is ReentrancyGuard, OwnerManagable {
    ...
    bytes32 public constant FASTWITHDRAWAL_APPROVAL_TYPEHASH =
        keccak256(
    ...
    constructor(FastWithdrawalAgent _agent) {
        ...
        DOMAIN_SEPARATOR = EIP712.hash(
            EIP712.Domain(
                "FastWithdrawalLiquidityProvider",
                "1.0",
                address(this)
            )
        );
    ...
    function execute(
        FastWithdrawalApproval[] calldata approvals
    ) external nonReentrant {
        // Prepare the data and validate the approvals when necessary
        FastWithdrawalAgent.Withdrawal[]
            memory withdrawals = new FastWithdrawalAgent.Withdrawal[](
                approvals.length
            );
        ...
        for (uint i = 0; i < approvals.length; i++) {
            require(
                skipApprovalCheck || isApprovalValid(approvals[i]),
                "PROHIBITED"
            );
    ...
    function isApprovalValid(
        ...
        return
            hash.verifySignature(approval.signer, approval.signature) &&
    ...
    function checkValidUntil(uint64 validUntil) internal view returns (bool) {
        return
            (validUntil & 0xFFFFFFFF) >= block.timestamp ||
            (validUntil >> 32) >= block.number;
    }

    function translate(
        FastWithdrawalApproval calldata approval
    ) internal pure returns (FastWithdrawalAgent.Withdrawal memory) {
        return
            FastWithdrawalAgent.Withdrawal({
