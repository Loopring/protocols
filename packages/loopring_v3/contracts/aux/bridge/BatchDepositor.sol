...
abstract contract BatchDepositor is IBatchDepositor, ReentrancyGuard {
    ...
    constructor(IExchangeV3 _exchange, uint32 _accountID) {
        exchange = _exchange;
    ...
    function batchDeposit(
        ...
        IBatchDepositor.Deposit[][]
            memory depositsList = new IBatchDepositor.Deposit[][](1);
    ...
    function withdrawFromPendingBatchDeposits(
        ...
        require(
            _arePendingDepositsTooOld(batchID, hash),
        ...
        for (uint i = 0; i < indices.length; i++) {
            ...
            address tokenAddress = exchange.getTokenAddress(
                deposits[idx].tokenID
            );

            tokenAddress.transferOut(deposits[idx].owner, deposits[idx].amount);
        }
    ...
    function _batchDeposit(
        ...
        require(
            totalNumDeposits <= MAX_NUM_TRANSACTIONS_IN_BLOCK,
            "MAX_DEPOSITS_EXCEEDED"
        );
        ...
        for (uint i = 0; i < depositsList.length; i++) {
            ...
            for (uint j = 0; j < _deposits.length; j++) {
                ...
                tokens[tokenIdx].amount = tokens[tokenIdx].amount.add(
                    deposit.amount
                );
                ...
                assembly {
                    ...
                    mstore(
                        transfersData,
                        or(shl(96, mload(deposit)), mload(add(deposit, 64)))
                    )
                    transfersData := add(transfersData, 34)
                }
            }
        ...
        assembly {
            transfersData := sub(
                transfersData,
                add(32, mul(34, totalNumDeposits))
            )
        }
        ...
        for (uint i = 0; i < numDistinctTokens; i++) {
            if (tokens[i].token == address(0)) {
                require(
                    tokens[i].amount == msg.value || from == address(this),
                    "INVALID_ETH_DEPOSIT"
                );
            }
    ...
    function _deposit(address from, address token, uint96 amount) internal {
        if (amount == 0) {
            return;
        }
    ...
    ) internal returns (uint16 cachedTokenID) {
    ...
    function _arePendingDepositsTooOld(
        uint batchID,
        bytes32 hash
