...
contract UniswapTokenSeller is ReentrancyGuard, ITokenSeller {
    ...
    constructor(address _uniswapFactoryAddress, address _recipient) {
    ...
    function sellToken(
        ...
        if (tokenS == address(0)) {
            ...
            if (tokenB == address(0)) {
                ...
                slippage = getSlippage(
                    exchangeB.getEthToTokenInputPrice(
                        exchange.getTokenToEthInputPrice(amountS)
                    ),
                    exchangeB.getEthToTokenInputPrice(
                        exchange.getTokenToEthInputPrice(amountS.mul(2))
    ...
    function getUniswapExchange(
        address token
    ) private view returns (UniswapExchangeInterface) {
        UniswapFactoryInterface factory = UniswapFactoryInterface(
