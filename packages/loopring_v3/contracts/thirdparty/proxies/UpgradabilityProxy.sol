...
contract UpgradeabilityProxy is Proxy {
    ...
    bytes32 private constant delayedImplementationPosition =
        keccak256("org.zeppelinos.proxy.delayedImplementation");
    ...
    function delayedImplementation()
        internal
        pure
        returns (DelayedImpl memory delayedImpl)
    {
        return _delayedImplementation();
    }

    function _delayedImplementation()
        internal
        pure
        returns (DelayedImpl storage delayedImpl)
    {
    ...
    function setDelayedImplementation(address newImplementation) internal {
        DelayedImpl storage delayedImpl = _delayedImplementation();
    ...
    function _applyUpgrade() internal {
        DelayedImpl storage delayedImpl = _delayedImplementation();
        require(
            delayedImpl.delayedImplementation != address(0),
            "NO_DELAYED_UPGRADE"
        );
        require(
            block.timestamp >= delayedImpl.nextEffectiveTime,
            "NOT_EFFECT_YET"
        );
        setImplementation(delayedImpl.delayedImplementation);

        delayedImpl.delayedImplementation = address(0);
        delayedImpl.nextEffectiveTime = 0;
    }
}
