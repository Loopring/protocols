...
library BatchVerifier {
        ...
        )
    {
        ...
        for (uint256 proofNumber = 0; proofNumber < num_proofs; proofNumber++) {
            if (proofNumber == 0) {
                ...
                // Safer entropy:
                entropy[proofNumber] = getProofEntropy(
                    in_proof,
                    proof_inputs,
                    proofNumber
                );
            ...
            for (uint256 i = 0; i < numPublicInputs; i++) {
                require(
                    proof_inputs[proofNumber * numPublicInputs + i] < q,
                    "INVALID_INPUT"
                );
    ...
    function prepareBatches(
        ...
        uint256[] memory inputAccumulators
    ) internal view returns (bool success, uint256[4] memory finalVksAlphaX) {
    ...
    function BatchVerify(
        ...
        uint256[] memory proofsAandC;
        uint256[] memory inputAccumulators;
        (valid, proofsAandC, inputAccumulators) = accumulate(
            in_proof,
            proof_inputs,
            num_proofs
        );
        ...
        (valid, finalVksAlphaX) = prepareBatches(
            in_vk,
            vk_gammaABC,
            inputAccumulators
        );
