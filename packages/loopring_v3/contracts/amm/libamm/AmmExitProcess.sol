...
library AmmExitProcess {
    ...
    function processExit(
        ...
        if (signature.length == 0) {
            if (S.exitMode) {
                ...
                bytes32 forcedExitHash = AmmExitRequest.hash(
                    ctx.domainSeparator,
                    S.forcedExit[exit.owner]
                );
            ...
            require(
                txHash.verifySignature(exit.owner, signature),
                "INVALID_OFFCHAIN_APPROVAL"
            );
        }

        (bool slippageOK, uint96[] memory amounts) = _calculateExitAmounts(
            ctx,
            exit
        );
    ...
    function _processExitTransfers(
        ...
        for (uint i = 0; i < ctx.tokens.length; i++) {
            ...
            (uint packedData, address to, address from) = AmmUtil.readTransfer(
                ctx
            );
            ...
            require(
                ...
                packedData & 0xffffffffffff00000000ffff0000000000000000000000 ==
                    (uint(ExchangeData.TransactionType.TRANSFER) << 176) |
                        (1 << 168) |
                        (uint(ctx.accountID) << 136) |
                        (uint(token.tokenID) << 88) &&
                    // transfer.amount.add(transfer.fee).isAlmostEqualAmount(amounts[i])
                    (100000 - 8) * targetAmount <= (amount + fee) * 100000 &&
                    (amount + fee) * 100000 <= (100000 + 8) * targetAmount &&
                    from == address(this) &&
                    to == exit.owner,
            ...
            if (fee > 0) {
                require(
                    i == ctx.tokens.length - 1 &&
                        /*feeTokenID*/ (packedData >> 48) & 0xffff ==
                        token.tokenID &&
                        fee <= exit.fee,
                    "INVALID_FEES"
                );
    ...
    function _burnPoolTokenOnL2(
        ...
        require(
            ...
            packedData & 0xffff00000000ffffffffffff000000ffffffff00000000 ==
                (uint(ExchangeData.TransactionType.TRANSFER) << 176) |
                    (1 << 168) |
                    (uint(ctx.accountID) << 104) |
                    (uint(ctx.poolTokenID) << 88) &&
                // transfer.amount.isAlmostEqualAmount(burnAmount) &&
                (100000 - 8) * burnAmount <= amount * 100000 &&
                amount * 100000 <= (100000 + 8) * burnAmount &&
                to == address(this) &&
                from == _from &&
                (signature.length == 0 ||
                    /*storageID*/ (packedData & 0xffffffff) == burnStorageID),
    ...
    function _calculateExitAmounts(
        AmmData.Context memory ctx,
        AmmData.PoolExit memory exit
    ) private pure returns (bool /* slippageOK */, uint96[] memory amounts) {
        amounts = new uint96[](ctx.tokens.length);

        // Calculate how much will be withdrawn
        uint ratio = uint(AmmData.POOL_TOKEN_BASE).mul(exit.burnAmount) /
            ctx.totalSupply;

        for (uint i = 0; i < ctx.tokens.length; i++) {
            uint96 amount = (ratio.mul(ctx.tokenBalancesL2[i]) /
                AmmData.POOL_TOKEN_BASE).toUint96();
        ...
        for (uint i = 0; i < ctx.tokens.length; i++) {
            ctx.vTokenBalancesL2[i] = (uint(ctx.vTokenBalancesL2[i]).mul(
                newTotalSupply
            ) / ctx.totalSupply).toUint96();
        }
