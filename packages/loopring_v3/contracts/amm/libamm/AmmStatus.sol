...
library AmmStatus {
    ...
    function setupPool(
        ...
        require(
            bytes(config.poolName).length > 0 &&
        ...
        S.domainSeparator = EIP712.hash(
            EIP712.Domain(config.poolName, "1.0.0", address(this))
        );
        ...
        if (S.accountID == 0) {
            ...
            S.allowance[address(this)][
                address(exchange.getDepositContract())
            ] = type(uint256).max;
            exchange.deposit(
                address(this), // from
                address(this), // to
    ...
    function shutdownByLP(AmmData.State storage S, address exitOwner) public {
    ...
    function shutdownByController(AmmData.State storage S) public {
        _shutdown(S);
    }
    ...
    function _shutdown(AmmData.State storage S) internal {
