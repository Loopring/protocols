    ...
    ReentrancyGuard
{
    ...
    function setupPool(
        ...
        require(
            state.accountID == 0 || msg.sender == address(controller),
            "UNAUTHORIZED"
        );
        state.setupPool(config);
    }

    function enterExitMode(bool enabled) external onlyFromController {
    ...
    function shutdown(
        address exitOwner
    ) external payable onlyWhenOnline nonReentrant {
    ...
    function withdrawWhenOffline() external onlyWhenOffline nonReentrant {
    ...
    function transferOut(
        ...
        address token,
        uint amount
    ) external nonReentrant onlyFromAssetManager {
    ...
    function getBalanceL1(address token) public view returns (uint96) {
