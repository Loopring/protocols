...
contract AmplifiedAmmController is IAmmController, Claimable {
    ...
    function getInitialVirtualBalances(
        ...
        for (uint i = 0; i < joinAmounts.length; i++) {
            vTokenBalancesL2[i] = (uint(joinAmounts[i]).mul(
                amplificationFactor
            ) / AMPLIFICATION_FACTOR_BASE).toUint96();
    ...
    function authorizeCurveChange(address pool) external onlyOwner {
        curveChangeAuthorization[pool] =
            block.timestamp +
            CURVE_CHANGE_MIN_DELAY;
    }
    ...
    function getAmplificationFactor(
        address pool
    ) public view returns (uint amplificationFactor) {
    ...
    function consumeCurveChangeAuthorized(
        address pool
