...
> [Feedback]: I dont' quite understand this. One question is whether the orderId need to be increment by 1 from 0 or it is a random number which is not safe). From certain perspective, the orderId is more like a virtual sub-account or a `order group id`.
...
> Here the `block` concept is different from a blockchain's block and an offchain chunk, right? We may need another term for it, such as "deposit/withdrawl window"?
...
> [Feedback] Will increasing the num of deposits/withdrawals increase the circuits? 8 seems too small.
...
> Questions: It seems to me a DEX can join any State without permission. But if an operator wants to join in an existing State, it should get permission from existing operators or the owner of the state.
> 
> Feedback: I recommand to rename `createNewState` to `createRealm`, `createAccountAndDeposit` to `createAccount`.
...
> 
> When we say DEX, do we refer to a wallet, or a wallet + ring-matcher. DEX as a term is confusing.
