...
const transaction_1 = __importDefault(
  require("../lib/wallet/ethereum/transaction")
);
...
class Exchange {
  ...
  createOrUpdateAccount(wallet, password, nonce, gasPrice, permission) {
    try {
      const keyPair = eddsa_1.EdDSA.generateKeyPair(
        wallet.getAddress() + password
      );
  ...
  createAccountAndDeposit(
    ...
    try {
      ...
      const data = wallet_1.ethereum.abi.Contracts.ExchangeContract.encodeInputs(
      ...
      return new transaction_1.default({
        ...
        gasLimit: fm.toHex(
          config_1.default.getGasLimitByType("create").gasInWEI
        )
      });
    } catch (err) {
  ...
  deposit(wallet, symbol, amount, nonce, gasPrice) {
    ...
    try {
      ...
      if (wallet.getAddress()) {
        ...
        if (symbol === "ETH") {
          to = "0x0";
          data = wallet_1.ethereum.abi.Contracts.ExchangeContract.encodeInputs(
            "deposit",
            {
              tokenAddress: to,
        ...
        return new transaction_1.default({
          ...
          gasPrice: fm.toHex(fm.fromGWEI(gasPrice)),
          gasLimit: fm.toHex(
            config_1.default.getGasLimitByType("depositTo").gasInWEI
          )
        });
      }
    } catch (err) {
    ...
    try {
      ...
      if (wallet.getAddress()) {
        ...
        return new transaction_1.default({
          ...
          gasPrice: fm.toHex(fm.fromGWEI(gasPrice)),
          gasLimit: fm.toHex(
            config_1.default.getGasLimitByType("withdrawFrom").gasInWEI
          )
        });
      }
    } catch (err) {
  ...
  submitWithdrawal(withdrawal) {
    ...
    withdrawal.amountInBN = config_1.default.toWEI(
      token.symbol,
      withdrawal.amount
    );
    ...
    withdrawal.amountFInBN = config_1.default.toWEI(
      feeToken.symbol,
      withdrawal.amountF
    );
    withdrawal.amountF = withdrawal.amountFInBN.toString(10);
    withdrawal.label =
      withdrawal.label !== undefined
        ? withdrawal.label
        : config_1.default.getLabel();
  ...
  setupOrder(order) {
    ...
    order.label =
      order.label !== undefined ? order.label : config_1.default.getLabel();
  ...
  submitCancel(cancel) {
    ...
    cancel.amountFInBN = config_1.default.toWEI(
      feeToken.symbol,
      cancel.amountF
    );
  ...
  submitFlexCancel(request) {
    ...
    const hash = fm.addHexPrefix(
      sha256_1.default(JSON.stringify(sign)).toString()
    );
    // Create signature
