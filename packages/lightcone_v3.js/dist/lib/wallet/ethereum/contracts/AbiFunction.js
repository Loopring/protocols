...
class AbiFunction {
  ...
  encodeInputs(inputs) {
    ...
    return (
      this.methodAbiHash +
      formatter_1.clearHexPrefix(
        formatter_1.toHex(abi.rawEncode(this.inputTypes, abiInputs))
      )
    );
  }
  /**
  ...
  decodeOutputs(outputs) {
    return this.parseOutputs(
      abi.rawDecode(this.outputTypes, formatter_1.toBuffer(outputs))
    );
  }
  /**
  ...
  decodeEncodedInputs(encoded) {
    return this.parseOutputs(
      abi.rawDecode(
        this.inputTypes,
        formatter_1.toBuffer(formatter_1.addHexPrefix(encoded))
      )
    );
  }
  parseInputs(inputs = {}) {
