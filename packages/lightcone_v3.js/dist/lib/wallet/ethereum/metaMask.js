    ...
    return new Promise(resolve => {
      web3.eth.sign(account, hash, function(err, result) {
        if (!err) {
          ...
          const v = formatter_1.toNumber(
            formatter_1.addHexPrefix(result.slice(130, 132))
          );
...
function signMessage(web3, account, message) {
  const hash = formatter_1.toHex(
    ethereumjs_util_1.hashPersonalMessage(ethereumjs_util_1.sha3(message))
  );
