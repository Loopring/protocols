...
function toFloat(value, encoding) {
  ...
  const maxExponentValue = new bn_js_1.default(encoding.exponentBase).pow(
    new bn_js_1.default(maxExponent)
  );
  ...
  while (
    r.gte(new bn_js_1.default(encoding.exponentBase)) ||
...
function fromFloat(f, encoding) {
  ...
  const value = new bn_js_1.default(mantissa).mul(
    new bn_js_1.default(encoding.exponentBase).pow(
      new bn_js_1.default(exponent)
    )
  );
  return value;
}
