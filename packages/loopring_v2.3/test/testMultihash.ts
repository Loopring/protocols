...
contract("Multihash", (accounts: string[]) => {
  ...
  //     {
  //       // Different hash
  //       const multiHashData = await util.signAsync(SignAlgorithm.Ethereum, new Buffer(hash2.slice(2), "hex"),
  //         signer1);
  //       const success = await multihash.verifySignature(signer1, hash1, multiHashData);
  //       assert(!success, "Signature should not be valid");
  //     }
  //     {
  //       // Different signer
  //       const multiHashData = await util.signAsync(SignAlgorithm.Ethereum, new Buffer(hash1.slice(2), "hex"),
  //         signer1);
