...
describe("proxy", () => {
  ...
  it("initial value of proxy", async function () {
    ...
    await proxy.upgradeTo(target.address);
    const proxyAsTarget = await (
      await ethers.getContractFactory("TestTargetContract")
    ).attach(proxy.address);
    const value = await proxyAsTarget.value();
  ...
  describe("fallback call", () => {
    it("proxy revert when proxy call returns 0", async () => {
      ...
      const priceOracleDelegate = await (
        await ethers.getContractFactory("PriceOracleDelegate")
      ).deploy(uniswapV2PriceOracle.address);

      const proxy = await (
        await ethers.getContractFactory("OwnedUpgradeabilityProxy")
      ).deploy();
      const proxyPriceOracle = await (
        await ethers.getContractFactory("UniswapV2PriceOracle")
      ).attach(proxy.address);
      ...
      const proxyDelegateOracle = await (
        await ethers.getContractFactory("PriceOracleDelegate")
      ).attach(proxy.address);
