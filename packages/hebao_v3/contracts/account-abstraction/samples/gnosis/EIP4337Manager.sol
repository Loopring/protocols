...
contract EIP4337Manager is IAccount, GnosisSafeStorage, Executor {
    ...
    function validateUserOp(
        ...
        address msgSender = address(
            bytes20(msg.data[msg.data.length - 20:])
        );
        require(
            msgSender == entryPoint,
    ...
    function executeAndRevert(
        ...
        bytes memory returnData = Exec.getReturnData(
            type(uint256).max
        );
    ...
    function validateEip4337(
        ...
        uint256 nonce = uint256(
            IEntryPoint(manager.entryPoint()).getNonce(
                address(safe),
                0
            )
        );
        ...
        userOps[0] = userOp;
        IEntryPoint _entryPoint = IEntryPoint(
            payable(manager.entryPoint())
        );
