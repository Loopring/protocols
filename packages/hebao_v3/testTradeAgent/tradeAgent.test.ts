...
describe('trade agent test', () => {
  ...
  describe('permission', () => {
    it('not approved executor should be rejected', async () => {
      ...
      const wETHConnector = await getVerifiedContractAt(
  ...
  describe('Compound connector', () => {
    const depositAndPrepare = async () => {
      ...
      const data = await depositData(
        'USDC-A',
        CONSTANTS.ONE_FOR_USDC,
        0,
        0
      )
      const balance1: BigNumber = await cERC20.balanceOf(
        smartWallet.address
      )
      const txReceipt = await userOpCast(
        [CONSTANTS.COMPOUND_CONNECTOR_ADDRESS],
      ...
      expect(msg).undefined
      const balance2: BigNumber = await cERC20.balanceOf(
        smartWallet.address
      )
      return {
        loadedFixture,
    ...
    it('withdraw', async () => {
      const { balance2, loadedFixture, cERC20, executor } =
      ...
      const msg = await getFirstUserOpErrMsg(txReceipt, entrypoint)
      expect(msg).undefined
      const balance3: BigNumber = await cERC20.balanceOf(
        smartWallet.address
      )
      expect(balance2.sub(balance3)).gt(0)
    })
    it('borrow', async () => {
      const { compConnector, loadedFixture, executor } =
        await depositAndPrepare()
      const { smartWallet, entrypoint } = loadedFixture
      ...
      const balance1: BigNumber = await USDT.balanceOf(
        smartWallet.address
      )
      const data = (
        await compConnector.populateTransaction.borrow(
      ...
      const msg = await getFirstUserOpErrMsg(txReceipt, entrypoint)
      expect(msg).undefined
      const balance2: BigNumber = await USDT.balanceOf(
        smartWallet.address
      )
      expect(balance2.sub(balance1)).eq(CONSTANTS.ONE_FOR_USDC.div(2))
    })
    it('repay', async () => {
      const { loadedFixture, executor } = await depositAndPrepare()
      ...
      const balance3: BigNumber = await USDT.balanceOf(
        smartWallet.address
      )
  ...
  it('WETH Connector', async () => {
    ...
    const balance1: BigNumber = await wETH.balanceOf(
      smartWallet.address
    )
    const wETHConnector = await getVerifiedContractAt(
      CONSTANTS.WETH_CONNECTOR_ADDRESS
    ...
    const balance2: BigNumber = await wETH.balanceOf(
      smartWallet.address
    )
  ...
  it('UniswapV2 Connector', async () => {
    ...
    const balance1: BigNumber = await UNI.balanceOf(
      smartWallet.address
    )
    const executor = await makeAnExecutor(
      [CONSTANTS.UniswapV2_CONNECTOR_ADDRESS],
    ...
    const msg2 = await getFirstUserOpErrMsg(txReceipt, entrypoint)
    expect(msg2).undefined
    const balance2: BigNumber = await UNI.balanceOf(
      smartWallet.address
    )
    expect(balance2.sub(balance1).gt(0)).true
  })
  ...
  describe('cast', () => {
    it('composed cast', async () => {
      ...
      const balance1: BigNumber = await WBTC.balanceOf(
        smartWallet.address
      )
      await userOpCast(
        [
  ...
  describe('intergaration', () => {
    const prepare = async (
      ...
      await faucetToken(
        CONSTANTS.USDC_ADDRESS,
        loadedFixture.smartWallet.address,
        '1'
      )
      await deposit(
        loadedFixture,
    ...
    it('sell Collateral token for borrow token and repay', async () => {
      ...
      const getState = async () => {
        ...
        const snapshot = await cUSDT.getAccountSnapshot(
          smartWallet.address
        )
      ...
      const cUSDT = await getVerifiedContractAt(
        CONSTANTS.cUSDT_ADDRESS
      )
      const executor = await makeAnExecutor(
        [
          CONSTANTS.COMPOUND_CONNECTOR_ADDRESS,
      ...
      const data1 = await withdrawData(
        'USDC-A',
        CONSTANTS.ONE_FOR_USDC.div(100),
        0,
        1
      )
