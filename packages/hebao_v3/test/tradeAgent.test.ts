...
describe("trade agent test", () => {
  ...
  describe("Compound connector", () => {
    const depositAndPrepare = async () => {
      ...
      const { smartWallet, smartWalletOwner, entrypoint, sendUserOp, create2 } =
        loadedFixture;
      const compConnector = await getVerifiedContractAt(
        COMPOUND_CONNECTOR_ADDRESS
      );
    ...
    it("withdraw", async () => {
      ...
      const { smartWallet, sendUserOp, create2, entrypoint } = loadedFixture;
  ...
  it("WETH Connector", async () => {
    ...
    const data2 = (
      await wETHConnector.populateTransaction.deposit(oneForETH, 0, 0)
    ).data;
    const txReceipt = await userOpCast(
      WETH_CONNECTOR_ADDRESS,
      data2,
  ...
  it("UniswapV2 Connector", async () => {
    ...
    // }
    const loadedFixture = await loadFixture(fixture);
    const { smartWallet, create2, entrypoint, sendUserOp, smartWalletOwner } =
      loadedFixture;
  ...
  describe("cast", () => {
    ...
    it("composed cast", async () => {
      ...
      const txScript = await userOpCast(
        [UniswapV2_CONNECTOR_ADDRESS, UniswapV2_CONNECTOR_ADDRESS],
        [swapData, swapData2],
        executor,
        loadedFixture
      );
  ...
  describe("intergaration", () => {
    ...
    it("sell Collateral token for borrow token and repay", async () => {
      ...
      const executor = await makeAExecutor(
        [COMPOUND_CONNECTOR_ADDRESS, UniswapV2_CONNECTOR_ADDRESS],
        loadedFixture
      );
    ...
    it("automation execution with non executor", async () => {
      ...
      const castAddresses = [COMPOUND_CONNECTOR_ADDRESS];
