...
import { createRandomAccount } from "./helper/utils";
...
const deposit = async (smartWallet: SmartWalletV3, tokenId: string,amount:BigNumber, getId: number, setId: number) => {
  const data = depositData(tokenId,amount,getId,setId);
...
describe.only("trade agent test", () => {
  ...
  describe('intergaration', () => {
    ...
    it('automation execution with non executor', async () => {
      ...
      expect(automationWithNonExecutor.cast(smartWallet.address, castAddresses, castDatas)).reverted;
    })
  })

  // it(('Do not add this test'), async () => {
  //   const {
  //     smartWallet,
  //     automation
  //   } = await loadFixture(fixture);
  //   const randomAcc = ethers.Wallet.createRandom().connect(smartWallet.provider)
  //   const impersonatedAutomationExecutor = await ethers.getImpersonatedSigner(AUTOMATION_EXCUTOR);
  //   // const smartWalletWithRandom = smartWallet.connect(randomAcc)
  //   const automationWithExecutor = automation.connect(randomAcc)
  ...
  //   ]
  //   const state2 = await getState()
  //   const r1 = await (await automationWithExecutor.cast(smartWallet.address, castAddresses, castDatas)).wait()
  //   const state3 = await getState()
  //   debugger
