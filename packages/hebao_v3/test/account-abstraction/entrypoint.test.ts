...
describe('EntryPoint', function () {
  ...
  before(async function () {
    ...
    expect(
      getUserOpHash(sampleOp, entryPoint.address, chainId)
    ).to.eql(await entryPoint.getUserOpHash(sampleOp))
  })
  ...
  describe('Stake Management', () => {
    ...
    it('should deposit for transfer into EntryPoint', async () => {
      ...
      expect(
        await entryPoint.balanceOf(await signer2.getAddress())
      ).to.eql(ONE_ETH)
      expect(
    ...
    describe('with stake of 2 eth', () => {
      ...
      describe('with unlocked stake', () => {
        ...
        it('should fail to withdraw before unlock timeout', async () => {
          await expect(
            entryPoint.withdrawStake(AddressZero)
          ).to.revertedWith('Stake withdrawal is not due')
        })
        ...
        describe('after unstake delay', () => {
          ...
          it('should succeed to withdraw', async () => {
            ...
            expect(await ethers.provider.getBalance(addr1)).to.eq(
              stake
            )
            const {
              stake: stakeAfter,
    ...
    describe('with deposit', () => {
      ...
      it('should be able to withdraw', async () => {
        ...
        expect(await account.getDeposit()).to.equal(
          depositBefore.sub(ONE_ETH)
        )
      })
    })
  })
  ...
  describe('#simulateHandleOp', () => {
    it('should simulate execution', async () => {
      ...
      await fund(account)
      const counter = await new TestCounter__factory(
        ethersSigner
      ).deploy()
      ...
      const ret = await entryPoint.callStatic
        .simulateHandleOp(
          userOp,
          counter.address,
          counter.interface.encodeFunctionData('counters', [
            account.address
          ])
        )
        .catch((e) => e.errorArgs)
  ...
  describe('flickering account validation', () => {
    ...
    it('should limit revert reason length before emitting it', async () => {
      ...
      const badData =
        await testRevertAccount.populateTransaction.revertLong(
          revertLength + 1
        )
    ...
    describe('warm/cold storage detection in simulation vs execution', () => {
      ...
      it('should prevent detection through getAggregator()', async () => {
        const testWarmColdAccount =
          await new TestWarmColdAccount__factory(ethersSigner).deploy(
            entryPoint.address,
            { value: parseEther('1') }
          )
        const badOp: UserOperation = {
        ...
        const beneficiaryAddress = createAddress()
        try {
          await entryPoint.simulateValidation(badOp, {
            gasLimit: 1e6
          })
        } catch (e: any) {
  ...
  describe('2d nonces', () => {
    ...
    describe('with key=1, seq=1', () => {
      ...
      it('should allow to increment nonce of different key', async () => {
        ...
        await entryPoint.callStatic.handleOps(
          [op],
          beneficiaryAddress
        )
      })
      ...
      it('should allow manual nonce increment', async () => {
        ...
        const incrementCallData =
          entryPoint.interface.encodeFunctionData('incrementNonce', [
            incNonceKey
          ])
        const callData = account.interface.encodeFunctionData(
          'execute',
          [entryPoint.address, 0, incrementCallData]
        )
        const op = await fillAndSign(
          {
            sender,
        ...
        expect(
          await entryPoint.getNonce(sender, incNonceKey)
        ).to.equal(BigNumber.from(incNonceKey).shl(64).add(1))
      })
  ...
  describe('without paymaster (account pays in eth)', () => {
    describe('#handleOps', () => {
      ...
      it('account should pay for tx', async function () {
        ...
        expect(countAfter.toNumber()).to.equal(
      ...
      it('account should pay for high gas usage tx', async function () {
        ...
        expect(await counter.offset()).to.equal(
          offsetBefore.add(iterations)
        )
      })
      ...
      it('account should not pay if too low gas limit was set', async function () {
        ...
        const inititalAccountBalance = await getBalance(
          account.address
        )
        const beneficiaryAddress = createAddress()
        const offsetBefore = await counter.offset()
        ...
        expect(await getBalance(account.address)).to.eq(
          inititalAccountBalance
        )
      })
      ...
      it('legacy mode (maxPriorityFee==maxFeePerGas) should not use "basefee" opcode', async function () {
        ...
        const ops = await debugTransaction(rcpt.transactionHash).then(
          (tx) => tx.structLogs.map((op) => op.op)
        )
      ...
      it('if account has a deposit, it should use it to pay', async function () {
        ...
        const depositBefore = await entryPoint.balanceOf(
          account.address
        )
        ...
        const depositAfter = await entryPoint.balanceOf(
          account.address
        )
      ...
      it('should fail to call recursively into handleOps', async () => {
        ...
        const execHandlePost = account.interface.encodeFunctionData(
          'execute',
          [entryPoint.address, 0, callHandleOps]
        )
        const op = await fillAndSign(
          {
            sender: account.address,
    ...
    describe('create account', () => {
      ...
      it('should succeed to create account after prefund', async () => {
        ...
        await expect(ret)
          ...
          .withArgs(
            ...
            createOp.sender,
            toChecksumAddress(
              ethers.utils.hexlify(createOp.initCode).slice(0, 42)
            ),
            AddressZero
          )
      ...
      it('should reject if account already created', async function () {
        ...
        await expect(
          entryPoint.callStatic.handleOps(
            [createOp],
            beneficiaryAddress,
            {
              gasLimit: 1e7
    ...
    describe('aggregation tests', () => {
      ...
      it('should fail to execute aggregated account with wrong aggregator', async () => {
        ...
        const wrongAggregator =
          await new TestSignatureAggregator__factory(
            ethersSigner
          ).deploy()
        const sig = HashZero
      ...
      it('should run with multiple aggregators (and non-aggregated-accounts)', async () => {
        ...
        const rcpt = await entryPoint
          .handleAggregatedOps(aggInfos, beneficiaryAddress, {
            gasLimit: 3e6
          })
        ...
        const events = rcpt.events
          ?.map((ev: Event) => {
            if (ev.event === 'UserOperationEvent') {
              return `userOp(${ev.args?.sender as string})`
            }
            if (ev.event === 'SignatureAggregatorChanged') {
              return `agg(${ev.args?.aggregator as string})`
            } else return null
          })
          .filter((ev) => ev != null)
      ...
      describe('execution ordering', () => {
        ...
        context('create account', () => {
          ...
          before(async () => {
            const factory =
              await new TestAggregatedAccountFactory__factory(
                ethersSigner
              ).deploy(entryPoint.address, aggregator.address)
          ...
          it('simulateValidation should return aggregator and its stake', async () => {
            ...
            expect(aggregatorInfo.aggregator).to.equal(
              aggregator.address
            )
            expect(aggregatorInfo.stakeInfo.stake).to.equal(TWO_ETH)
            expect(aggregatorInfo.stakeInfo.unstakeDelaySec).to.equal(
              3
            )
          })
    ...
    describe('Validation time-range', () => {
      ...
      before('init account with session key', async () => {
        ...
        account = await new TestExpiryAccount__factory(
          ethersSigner
        ).deploy(entryPoint.address)
      ...
      describe('validatePaymasterUserOp with deadline', function () {
        ...
        describe('time-range overlap of paymaster and account should intersect', () => {
          ...
          it('should use lower "after" value of account', async () => {
            expect(
              (await simulateWithPaymasterParams(200, 1000))
                .validAfter
            ).to.eql(200)
          })
