...
import { BadEntryPoint__factory, EntryPoint__factory, EntrypointReentryAttacker__factory } from "../typechain-types";
import { utils } from "ethers";

describe("reentry attack test", () => {

  it("reentry attack bad implemented entrypoint", async () => {
    const { deployer, create2 } =
    ...
    const attack = await new EntrypointReentryAttacker__factory(deployer).deploy(entryPoint)
    ...
    expect(balanceBefore.sub(balanceAfter).gt(utils.parseEther('10'))).to.be.true;
  });
