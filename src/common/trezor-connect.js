...
this.TrezorConnect = (function ()
{
    ...
    let IS_CHROME_APP = chrome && chrome.app && chrome.app.window;

    let ERR_TIMED_OUT = 'Loading timed out';
    let ERR_WINDOW_CLOSED = 'Window closed';
    let ERR_WINDOW_BLOCKED = 'Window blocked';
    let ERR_ALREADY_WAITING = 'Already waiting for a response';
    let ERR_CHROME_NOT_CONNECTED = 'Internal Chrome popup is not responding.';

    let DISABLE_LOGIN_BUTTONS = window.TREZOR_DISABLE_LOGIN_BUTTONS || false;
    let CHROME_URL = window.TREZOR_CHROME_URL || './chrome/wrapper.html';
    let POPUP_ORIGIN = window.TREZOR_POPUP_ORIGIN || 'https://connect.trezor.io';
    let POPUP_PATH = window.TREZOR_POPUP_PATH || POPUP_ORIGIN + '/' + TREZOR_CONNECT_VERSION;
    let POPUP_URL = window.TREZOR_POPUP_URL || POPUP_PATH + '/popup/popup.html?v=' + new Date().getTime();

    let POPUP_INIT_TIMEOUT = 15000;
   ...
   */
     ...
     */
        ...
        {
            let wrapperCallback = function (result)
            {
                if (result.success)
                {
     ...
     */
        ...
        {
            let elements = document.getElementsByTagName('trezor:login');

            for (let i = 0; i < elements.length; i++)
            {
                let e = elements[i];
                let text = e.getAttribute('text') || 'Sign in with TREZOR';
                let callback = e.getAttribute('callback') || '';
                let hosticon = e.getAttribute('icon') || '';
                let challenge_hidden = e.getAttribute('challenge_hidden') || '';
                let challenge_visual = e.getAttribute('challenge_visual') || '';
   ...
   */
    ...
    function Popup (url, origin, name, width, height)
    {
      ...
      ',personalbar=no' +
      ',status=no';
        let w = window.open(url, name, opts);

        let interval;
        let blocked = w.closed;
    ...
    function Channel (popup, waiting)
    {
        ...
        {
            let org1 = event.origin.match(/^.+\:\/\/[^\鈥屸€�/]+/)[0];
            let org2 = popup.origin.match(/^.+\:\/\/[^\鈥屸€�/]+/)[0];
    ...
    function PopupManager ()
    {
        ...
        this.sendWithChannel = function (message, callback)
        {
            ...
            {
                let succ = response.success && this.closeAfterSuccess;
                let fail = !response.success && this.closeAfterFailure;
                if (succ || fail)
                {
                    this.close();
                }
            ...
            let onchannel = function (channel)
            {
