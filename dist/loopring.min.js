...
"use strict";function Order(e){const t=e.protocol,r=e.owner,n=e.tokenS,o=e.tokenB,i=e.amountS,s=e.amountB,a=e.timestamp,u=e.ttl,g=e.salt,l=e.lrcFee,m=e.buyNoMoreThanAmountB,c=e.marginSplitPercentage;let S=e.v,h=e.r,d=e.s;const B=Joi.object().keys({protocol:Joi.string().regex(/^0x[0-9a-fA-F]{40}$/i),owner:Joi.string().regex(/^0x[0-9a-fA-F]{40}$/i),tokenS:Joi.string().regex(/^0x[0-9a-fA-F]{40}$/i),tokenB:Joi.string().regex(/^0x[0-9a-fA-F]{40}$/i),buyNoMoreThanAmountB:Joi.boolean(),marginSplitPercentage:Joi.number().integer().min(0).max(100),r:Joi.number().integer().min(0),s:Joi.string().regex(/^0x[0-9a-fA-F]{64}$/i),v:Joi.string().regex(/^0x[0-9a-fA-F]{64}$/i)}).with("protocol","owner","tokenS","tokenB","buyNoMoreThanAmountB","marginSplitPercentage").without("r","s","v"),N=["address","address","address","address","uint","uint","uint","uint","uint","uint","bool","uint8"];this.sign=function(b){if(!Joi.validate(e,B))throw new Error("Invalid Loopring Order");const x=signer.solSHA3(N,[t,r,n,o,new BN(Number(i).toString(10),10),new BN(Number(s).toString(10),10),new BN(Number(a).toString(10),10),new BN(Number(u).toString(10),10),new BN(Number(g).toString(10),10),new BN(Number(l).toString(10),10),m,c]),w=ethUtil.hashPersonalMessage(x);_.isString(b)&&(b=ethUtil.toBuffer(b));const p=ethUtil.ecsign(w,b);return S=Number(p.v.toString()),h="0x"+p.r.toString("hex"),d="0x"+p.s.toString("hex"),{protocol:t,owner:r,tokenS:n,tokenB:o,amountS:i,amountB:s,timestamp:a,ttl:u,salt:g,lrcFee:l,buyNoMoreThanAmountB:m,marginSplitPercentage:c,v:S,r:h,s:d}},this.cancel=function(e,t){h&&S&&d||this.sign(t);const B={addresses:[r,n,o],orderValues:[i,s,a,u,g,l,e],buyNoMoreThanAmountB:m,marginSplitPercentage:c,v:S,r:h,s:d};return signer.generateCancelOrderData(B)}}const signer=require("./signer.js"),ethUtil=require("ethereumjs-util"),_=require("lodash"),BN=require("bn.js"),Joi=require("joi");module.exports=Order;

},{"./signer.js":"signer","bn.js":3,"ethereumjs-util":"ethereumjs-util","joi":53,"lodash":79}],"relay":[function(require,module,exports){
