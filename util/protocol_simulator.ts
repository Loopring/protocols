...
export class ProtocolSimulator {
  ...
  private caculateNextFillAmountS(currentOrder: Order,
    ...
    let currentFillAmountB = currentFillAmountS * currentOrder.params.amountB.toNumber() / currentRateAmountS;

    let nextFillAmountS = nextRateAmountS;
    if (!nextOrder.params.buyNoMoreThanAmountB) {
      nextFillAmountS = nextOrder.params.amountS.toNumber();
    }
    ...
    for (let i = 0; i < size; i++) {
      ...
      if (0 == this.feeSelectionList[i]) {
        ...
        if (order.params.buyNoMoreThanAmountB) {
          feeItem.feeS = fillAmountSList[i] * order.params.amountS.toNumber() / rateAmountSList[i] - fillAmountSList[i];
