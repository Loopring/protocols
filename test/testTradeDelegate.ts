...
contract("TradeDelegate", (accounts: string[]) => {
  ...
  beforeEach(async () => {
    ...
    dummyExchange1 = await DummyExchange.new(tradeDelegate.address);
    dummyExchange2 = await DummyExchange.new(tradeDelegate.address);
    dummyExchange3 = await DummyExchange.new(tradeDelegate.address);
