...
contract("Exchange_Broker", (accounts: string[]) => {
  ...
  const submitRingsAndSimulate = async (context: pjs.Context, ringsInfo: pjs.RingsInfo, eventFromBlock: number) => {
    ...
    exchangeTestUtil.assertTransfers(ringsInfo, transferEvents, report.transferItems);
    await exchangeTestUtil.assertFeeBalances(ringsInfo, report.feeBalances);
    await exchangeTestUtil.assertFilledAmounts(ringsInfo, context, report.filledAmounts);
