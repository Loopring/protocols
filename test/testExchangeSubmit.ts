...
contract("Exchange_Submit", (accounts: string[]) => {
  ...
  //     return;
  //   }
  //   const tokenSymbol = exchangeTestUtil.testContext.tokenSymbolAddrMap.get(payment.token);
  ...
  const submitRingsAndSimulate = async (context: pjs.Context, ringsInfo: pjs.RingsInfo, eventFromBlock: number) => {
    ...
    exchangeTestUtil.assertEqualsRingsInfo(deserializedRingsInfo, ringsInfo);
    ...
    try {
      ...
      exchangeTestUtil.logDetailedTokenTransfers(ringsInfo, report);
    } catch (err) {
    ...
    const transferEvents = await exchangeTestUtil.getTransferEvents(allTokens, eventFromBlock);
    exchangeTestUtil.assertTransfers(deserializedRingsInfo, transferEvents, report.transferItems);
    await exchangeTestUtil.assertFeeBalances(deserializedRingsInfo, report.feeBalances);
    await exchangeTestUtil.assertFilledAmounts(deserializedRingsInfo, context, report.filledAmounts);
  ...
  //     const tokenSymbol = exchangeTestUtil.testContext.tokenSymbolAddrMap.get(t[0]);
  ...
  //       const tokenSymbol = exchangeTestUtil.testContext.tokenSymbolAddrMap.get(token);
